...
test_that("getFMort", {
    ...
    expect_identical(f1, fmg11)
    expect_identical(f2, fmg22)
    expect_identical(f3, fmg33)
})


# The following is commented out because we no longer want to produce the same
# results as the old mizer code but prefer to get more correct results that
# agree better with analytic results.
#
# test_that("Test that fft based integrator gives the same result as old code",{
#     

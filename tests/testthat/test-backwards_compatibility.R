...
test_that("MizerParams() works as in version 1", {
    ...
    #     scale_y_log10()
    # max(abs(old - new))
    expect_known_value(getPhiPrey(params, n = initialN(params), n_pp = initialNPlankton(params)), 
                       "values/set_multispecies_model_phi_prey",
                       check.attributes = FALSE,
                       tolerance = 0.0005)
    # The discrepancy is small, see the following graph:
    # new <- getPhiPrey(params, n = initialN(params), n_pp = initialNPlankton(params))
    # df <- melt(new) %>% mutate(type = "new")

test_that("plotYieldObservedVsModel works", {
    ...
    dummy = plotYieldObservedVsModel(params, return_data = T)
    
    # Check yields equal those put in
    expect_equal(dummy$observed, species_params(params)$yield_observed)
    ...
    dummy = plotYieldObservedVsModel(params2, return_data = T)
    expect_equal(as.character(dummy$species), 
                 species_params(params)$species[!is.na(species_params(params2)$yield_observed)])
    expect_equal(dummy$observed, 
                 species_params(params2)$yield_observed
                 [!is.na(species_params(params2)$yield_observed)])
    ...
    dummy = plotYieldObservedVsModel(params, species = sp_select, return_data = T)
    ...
    dummy = plotYieldObservedVsModel(params, return_data = T)
    p <- plotYieldObservedVsModel(params)

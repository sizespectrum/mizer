...
test_that("axis limits are set correctly", {
    ...
    expect_equal(p$scales$scales[[1]]$limits, c(1e3, 1e6))
    
    # Test with NA values
    p <- plotBiomass(sim, species = species, ylim = c(NA, 1e6))
    expect_equal(p$scales$scales[[1]]$limits[2], 1e6)
    ...
    expect_equal(p$scales$scales[[2]]$limits, c(1, 100))
    expect_equal(p$scales$scales[[1]]$limits, c(1e5, 1e8))
    
    # Test with NA values in wlim
    p <- plotSpectra(sim, species = species, wlim = c(10, NA), ylim = c(NA, 1e8))
    expect_equal(p$scales$scales[[2]]$limits[1], 10)
    expect_true(is.na(p$scales$scales[[2]]$limits[2]))
    expect_true(is.na(p$scales$scales[[1]]$limits[1]))
    expect_equal(p$scales$scales[[1]]$limits[2], 1e8)
})

...
custom reproduction rate function (RDD function). The RDD function
receives the current time `t` as an argument. We use this to turn
reproduction on (smoothly) only during a short window at the start of
...
sim_no_diff <- project(params, t_max = 5, dt = 0.005,

...
This guide covers projecting a model forward with project(): its
arguments, the four ways of specifying fishing effort, continuing and
comparing runs, and the choice of numerical scheme.

[`project()`](https://sizespectrum.org/mizer/reference/project.md)
advances a
[`MizerParams`](https://sizespectrum.org/mizer/reference/MizerParams.md)
object through time and returns a
[`MizerSim`](https://sizespectrum.org/mizer/reference/MizerSim.md). The
params object must already be set up and (usually) at steady state — see
the [guide to building a mizer
model](https://sizespectrum.org/mizer/articles/guide-build-model.md) and
the [guide to reaching steady state and
...
For form 4, build a `time × gear` matrix with **numeric, increasing**
...
Each effort value applies from its time until the next time in the
array. With an array, the simulation starts at the smallest time; use
`t_max` to extend beyond the last row.

To change *which* gears exist, or their selectivity and catchability,
before running, see the [guide to setting up
...
**Continue a simulation.** Pass a `MizerSim` back to
...
**Carry a simulation’s state into a fresh `MizerParams`** — to start a
...
[`setInitialValues(params, sim)`](https://sizespectrum.org/mizer/reference/setInitialValues.md).

**Scenario comparison.** Project the same params under different
efforts, then compare with the plotting tools (see the [guide to
...
For steady states and slow biomass or yield trends the defaults are
fine. But the default flux scheme (first-order **upwind**) carries
substantial *numerical diffusion*: it smears out cohorts and travelling
waves and can **silently damp a real oscillation or limit cycle down to
a flat line** — with no error to warn you. This is a correctness issue,
not just an accuracy one, so for **any study of dynamics**
(oscillations, cohort or wave structure, generation cycles, real
growth-diffusion effects) switch to the second-order scheme:

- **Space:** build the model with `second_order_w = TRUE` (the van Leer,
  flux-limited scheme) — set it in
  [`newMultispeciesParams(..., second_order_w = TRUE)`](https://sizespectrum.org/mizer/reference/newMultispeciesParams.md),
  or on an existing model with
  [`second_order_w(params) <- TRUE`](https://sizespectrum.org/mizer/reference/second_order_w.md).
  Because it changes the discrete steady state it lives in the params
  object, not in a
  [`project()`](https://sizespectrum.org/mizer/reference/project.md)
  argument.
...
`second_order_w = TRUE` but disappears (settles to a flat steady state)
under the default upwind scheme is being killed by numerical diffusion,
not by real dynamics.
...
`bin_average`, which switches every integral over the size grid from a
left-edge Riemann sum to a proper bin average. Unnormalised quantities —
biomass, yield,
[`getDiet(proportion = FALSE)`](https://sizespectrum.org/mizer/reference/getDiet.md)
— therefore shift by roughly `(1 + beta) / 2`, about 10% for a typical
grid, where `beta` is the ratio between neighbouring grid points. That
is the more accurate integral, not a regression. Proportions and ratios
are largely unaffected because the factor cancels. Set the two
independently if you want the better flux scheme without moving your
summary numbers:
...
The scheme lives in the `MizerParams`, so a `MizerSim` carries it:
...
to separate an internal instability from a boundary or reproduction one
...
- With growth diffusion switched on (`D_ext > 0`), diffusion spreads
  ...
  analyse, so that abundance at the boundary stays negligible; the
  default `1.5 * w_inf` is usually enough, but raise it if `D_ext` is
  large.
- `t_save` controls output resolution, not accuracy — the model always
  steps at `dt` internally.
- The `MizerSim` stores the `MizerParams` it used, so a sim is

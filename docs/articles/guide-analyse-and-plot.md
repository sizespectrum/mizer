...
`params@w` for biomass, or 1 for numbers); it selects the size range,
uses the quadrature scheme the model is actually on and wraps the result
in the appropriate mizer array class. Doing the sum by hand instead
means getting all of that right yourself, silently and only for some
users. Three things are worth knowing:

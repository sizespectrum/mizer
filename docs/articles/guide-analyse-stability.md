...
the per-species biomass series sampled at `t_save`, on every block
rather than only when the distance criterion has failed, because a cycle
whose period divides `t_per` is sampled at one phase and looks

...
- `extensions` A named list with one entry for each extension package
  needed to run the model. Each entry records the installation
  `requirement` and the package `version` whose object layout is in use.
  This field is set by extension packages with
  [`recordExtension()`](https://sizespectrum.org/mizer/reference/recordExtension.md)
  and can be inspected with `getMetadata(params)$extensions`.

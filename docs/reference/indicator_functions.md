...
| [`getMeanLength()`](https://sizespectrum.org/mizer/reference/getMeanWeight.md) | A vector with values at each saved time step (or a single value for MizerParams). | The mean length of the community through time. This is calculated as the total length of all individuals divided by their number, using the length-weight relationship of each species. |

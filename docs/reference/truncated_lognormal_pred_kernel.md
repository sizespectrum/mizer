...
but with an imposed maximum predator/prey mass ratio
...
\\w/w_p\\ is between 1 and \\\beta_i\exp(3\sigma_i)\\ and zero
otherwise. Here \\\beta_i\\ is the preferred predator-prey mass ratio
and \\\sigma_i\\ determines the width of the kernel. These two
parameters need to be given in the species parameter dataframe in the
columns `beta` and `sigma`.

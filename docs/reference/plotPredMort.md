...
After running a projection, plot the predation mortality rate of each
species by size. The mortality rate is averaged over the specified time
range (a single value for the time range can be used to plot a single
time step).
...
- all.sizes:
  ...
  If TRUE, then predation mortality is plotted also for sizes outside a
...
- ...:
  ...
  **For `MizerSim` methods:**
  ...
  :   The time range (either a vector of values, a vector of min and max
      time, or a single value) to average the predation mortality over.
      Default is the final time step.

## Value

A ggplot2 object, unless `return_data = TRUE`, in which case a data
frame with the three variables 'w' (or 'l' if `size_axis = "l"`),
'value', 'Species' is returned.

...
`MizerSim` objects in a single plot. Colours identify species or groups
and linetype identifies the object.
...
- ...:
  ...
  [`plotCDF()`](https://sizespectrum.org/mizer/reference/plotCDF.md) for
  preparing the cumulative distribution data, for example `time_range`
  or `geometric_mean` for `MizerSim` objects.

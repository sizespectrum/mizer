...
- A **metadata-only** extension registers itself in `params@extensions` for
  ...
  [mizerStarvation](https://github.com/sizespectrum/mizerStarvation) is an
...
- A **dispatching** extension additionally defines a new object type so that
  ...
  [mizerShelf](https://github.com/sizespectrum/mizerShelf) is an example: it
...
- **Testing and documentation**: a package gives you a natural home for
  `testthat` tests and roxygen2 documentation.
...
fish's energy balance is negative. It does this via the `other_mort` slot: a
named list of function names that `getMort()` calls and sums at every time step.
...
mizer when it is loaded. Place a `.onLoad` function in a file such as `R/zzz.R`:
...
the `"owner/repo"` form (e.g. `"sizespectrum/mizerStarvation"`). The call is safe to repeat: if the package is
...
everything that has been registered via `.onLoad` hooks since R started.
...
- When the object is loaded from disk with `readParams()`, mizer checks that
  all the recorded extensions are installed in the current session and warns
  the user if any are missing or outdated.
- `readParams()` also calls `coerceToExtensionClass()` automatically, using
  `@extensions` to decide which S4 class to restore the object to.
...
extension was also loaded, both appear in the chain. An object is always
coerced to the class of the outermost extension in its own chain, so an object
created with only `mizerShelf` registered will remain a `mizerShelf` object
even if `mizerOuter` is also loaded in the current session.
...
extension that provides a dispatch class, and promotes the object to that S4
class. After the call, `params` is of class `c("mizerShelf", "MizerParams")`,
and R will dispatch to `getBiomass.mizerShelf` automatically.
...
- [ ] Store all extension-specific state in `other_params(params)` or
  component parameters, not in new S4 slots.
...
- `vignette("extensions", package = "mizer")` for the full menu of extension
  mechanisms (custom rate functions, components, subclassing, and more).
- `?registerExtension`

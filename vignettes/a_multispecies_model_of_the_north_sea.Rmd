...
The species in the model are: `r paste(species_params[,"species"], collapse=", ")`,
which account for about 90% of the total biomass of all species sampled by
research trawl surveys in the North Sea. The `species_params` object is a
...
already in the `species_params` data.frame. The values were found through a
...
the other parameters apart from `kappa`, the carrying capacity of the plankton
spectrum (see [see the section on plankton density](model_description.html#plankton-density)). 
...
first historical year until equilibrium is reached. We then can carry on
projecting forward using the remaining years of effort. This approach reduces
the impact of transient dynamics.

Here we make an initial effort matrix of 100 years at the first effort level. We
need to include dimension names for the time dimension. We then stick it on top
of the original matrix of historical relative effort using `rbind()`.
...
This includes an initial 100 years of constant relative effort at the 1957
level, followed by the relative effort from 1957 to 2010. We use this effort
...
time. You can see the 100 year period of transients with constant fishing
fishing mortality before the historical relative mortality is used from 1967.
...
the unexploited community. Therefore we also project forward for 100 years with
0 fishing effort.
...
[the section about indicator functions for MizerSim objects.](exploring_the_simulation_results.html#functions-for-calculating-indicators)
...
community (we are only interested in the fishing history years, 1967 to 2010,
ignoring the transients):
...
sim1 <- project(params, effort = scenario1, dt = 0.25, t_save = 1)

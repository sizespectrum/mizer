...
set to be constant. The plankton spectrum only extends to the start of the
...
functions and the parameters of the plankton spectrum.
...
external mortality), `alpha` (the assimilation efficiency of the community),
...
<!-- page](../reference/MizerParams-class.html). The slots -->
...
been set from $`r min(params@w)`$ to $`r max(params@w)`$ and this is divided
into $`r length(params@w)`$ size bins. Similar information is available for the
...
arguments passed to the `newCommunityParams()` function. The plankton and
...
plot(x = sim@params@w, y = pred_mort[51,], log = "xy", type = "l", 
...
time, mizer provides the function `final_n()`, so we could have done
```{r label='relative_comm_abundance_simple'}
relative_abundance <- final_n(sim1) / final_n(sim0)
```

This can then be plotted using basic R plotting commands.
The sizes are stored in the `params@w` slot.


```{r label=plot_relative_comm_abund}
plot(x = params@w, y = relative_abundance, log = "x", type = "n",
    xlab = "Size (g)", ylab = "Relative abundance")
lines(x = params@w, y = relative_abundance)
lines(x = c(min(params@w), max(params@w)), y = c(1, 1), lty = 2)
```
...
As you can see in the underlying model equations in [the model description
section](model_description.html), the model runs in continuous time. Therefore,

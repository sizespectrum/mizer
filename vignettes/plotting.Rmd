...
package](https://ggplot2.tidyverse.org/) and the  [plotly package for
R](https://plot.ly/r/) to visualise the results from mizer simulations.
```{r}
library(mizer)
library(ggplot2)
library(plotly)
...
In plotly we use the `layout()` funcction.
```{r warning=FALSE}
pp %>% layout(
    title = "Biomass plot",
    xaxis = list(
...
```{r}
final_n <- n(sim)[final_idx(sim), , , drop = FALSE]
```
...
of the `r nrow(nf)` entries in the `1` x `r dim(n_final)[2]` x
`rdim(n_final)[3]` matrix `final_n`.
```{r}
str(nf)
```
...
```
This leaves a dataframe with only `r nrow(nf)` observations. 
...
## Including plankton spectrum

We can include additional lines in the plot by merging several data frames. For
example, we can include another line for the plankton spectrum. We first
convert also the plankton abundance at the final time into a data frame and
filter out the zero values
```{r}
nf_pp <- melt(n_pp(sim)[final_idx, , drop = FALSE]) %>% 
    filter(value > 0)
```
This data frame only contains three variables, because it does not have the 
`sp` column specifying the species. We add this column with the value "Plankton"
```{r}
nf_pp$sp <- "Plankton"
```
...
Comparision will be easier if we combine these two plots.
For that we add an extra variable to the dataframes that allow us to 
...
plotly is less good at faceting, but it can put arbitray plots
...
Also in plotly we can now tie the `effort` variable to the linetype.
```{r warning=FALSE}
plot_ly(yield) %>% 

...
Objects of class `MizerParams` are created using the constructor method `set_multispecies_model()` (this constructor method was called MizerParams() in previous version of mizer).
...
In the rest of this section we look at the main arguments to the `set_multispecies_model()` function.
...
This mean that if an `h` column is not included in the species parameter data.frame, a column for `k_vb` is necessary. If it is not included then the `set_multispecies_model()` method will fail with an error message. The calculation also requires a value of the feeding level of small individuals. This can be passed as an additional argument, `f0`, to the `set_multispecies_model()` constructor and it has a default value of 0.6.
...
If an interaction matrix is not specified to the `set_multispecies_model()` constructor the default interaction matrix is used. This has all values set to 1. 



## The other `set_multispecies_model()` arguments {#sec:other_mizerparams_arguments}
...
The arguments can be seen in the `set_multispecies_model()` help page.
...
This means that depending on which columns have been provided in the species parameters data.frame, some of the parameters from the the `set_multispecies_model()` help page may not be used. For example, if the column `z0` (the mortality from other sources) has been included, then the arguments `z0pre` and `z0exp` are not used.
...
As mentioned in the preceding sections, an object of `MizerParams` is created by using the `set_multispecies_model()`
...
This species parameter data.frame is the minimum we need to create a `MizerParams` object as it contains only essential columns. To create the `MizerParams` object we pass the data.frame into the `set_multispecies_model()` constructor method:
...
We will use default values for the remainder of the arguments to the `set_multispecies_model()` method. 

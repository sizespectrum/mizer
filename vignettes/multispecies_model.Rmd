...
Objects of class `MizerParams` are created using the constructor method `multispeciesParams()` (this constructor method was called MizerParams() in previous version of mizer).
...
In the rest of this section we look at the main arguments to the `multispeciesParams()` constructor method.
...
This mean that if an `h` column is not included in the species parameter data.frame, a column for `k_vb` is necessary. If it is not included then the `multispeciesParams()` method will fail with an error message. The calculation also requires a value of the feeding level of small individuals. This can be passed as an additional argument, `f0`, to the `multispeciesParams()` constructor and it has a default value of 0.6.
...
If an interaction matrix is not specified to the `multispeciesParams()` constructor the default interaction matrix is used. This has all values set to 1. 



## The other `multispeciesParams()` arguments {#sec:other_mizerparams_arguments}
...
The arguments can be seen in the `multispeciesParams()` help page.
...
This means that depending on which columns have been provided in the species parameters data.frame, some of the parameters from the the `multispeciesParams()` help page may not be used. For example, if the column `z0` (the mortality from other sources) has been included, then the arguments `z0pre` and `z0exp` are not used.
...
As mentioned in the preceding sections, an object of `MizerParams` is created by using the `multispeciesParams()`
...
Of these parameters, `species`, `w_inf`, `w_mat`, `beta` and `sigma` are essential and have no default values (as described in [the section on species parameters](model_description.html#sec:species_parameters_dataframe)).
`r_max` is a SRR parameter. We are going to use the default Beverton-Holt type SRR which has `r_max` as an argument (see [the section on the stock-recruitment relationship](multispecies_model.html#sec:recruitment)), making this column also essential.
...
This means that the default selectivity function, `knife_edge`, will be used. As mentioned in [the section on fishing gears](multispecies_model.html#fishing_gear),
...
This species parameter data.frame is the minimum we need to create a `MizerParams` object as it contains only essential columns. To create the `MizerParams` object we pass the data.frame into the `multispeciesParams()` constructor method:
...
We will use default values for the remainder of the arguments to the `multispeciesParams()` method. 
...
Now that we know how to create a multispecies model we shall discuss how to [run a multispecies model.](running_a_simulation)

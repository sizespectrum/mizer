...
Objects of class `MizerParams` are created using the constructor method `MizerParams()` (this constructor method was called MizerParams() in previous version of mizer).
...
In the rest of this section we look at the main arguments to the `MizerParams()` function.
...
The arguments can be seen in the `MizerParams()` help page.
...
This means that depending on which columns have been provided in the species parameters data.frame, some of the parameters from the the `MizerParams()` help page may not be used. For example, if the column `z0` (the mortality from other sources) has been included, then the arguments `z0pre` and `z0exp` are not used.

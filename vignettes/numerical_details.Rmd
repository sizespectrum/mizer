...
$$ \Delta w_j \frac{\partial N_j}{\partial t} + J(w_{j+1}) - J(w_j) = -\int_{w_j}^{w_{j+1}} \mu(w) N(w) dw $$
Approximating the integral and dividing by $\Delta w_j$:
$$ \frac{N_j^{t+1} - N_j^t}{\Delta t} + \frac{J_{j+1} - J_j}{\Delta w_j} = -\mu(w_j) N_j^{t+1} $$
...
$$ J_j^{diff} \approx -\frac{1}{2} \frac{(d N)_j - (d N)_{j-1}}{w_j - w_{j-1}} = -\frac{1}{2} \frac{d(w_j) N_j - d(w_{j-1}) N_{j-1}}{\Delta w_{j-1}} $$
...
$$ J_j = g(w_{j-1}) N_{j-1} - \frac{1}{2} \frac{d(w_j) N_j - d(w_{j-1}) N_{j-1}}{\Delta w_{j-1}} $$
And similarly at boundary $w_{j+1}$:
$$ J_{j+1} = g(w_j) N_j - \frac{1}{2} \frac{d(w_{j+1}) N_{j+1} - d(w_j) N_j}{\Delta w_j} $$

## Limitation on Time Step

With the diffusion term, an explicit time discretisation would require a very small time step for stability ($\Delta t \sim \Delta w^2$). Therefore, we use a semi-implicit scheme where the densities $N$ are evaluated at time $t+1$, but the rates ($g$, $\mu$, $d$) are evaluated at time $t$.

## Discretised Equation

Substituting the fluxes into the conservation equation:
$$
\frac{N_j^{t+1} - N_j^t}{\Delta t} + \frac{1}{\Delta w_j} \left( J_{j+1}^{t+1} - J_j^{t+1} \right) = -\mu(w_j) N_j^{t+1}
$$
...
$$
\begin{aligned}
A_j &= -\frac{\Delta t}{\Delta w_j} \left( g(w_{j-1}) + \frac{1}{2} \frac{d(w_{j-1})}{\Delta w_{j-1}} \right) \\
C_j &= -\frac{\Delta t}{\Delta w_j} \left( \frac{1}{2} \frac{d(w_{j+1})}{\Delta w_j} \right) \\
B_j &= 1 + \Delta t \mu(w_j) + \frac{\Delta t}{\Delta w_j} \left( g(w_j) + \frac{1}{2} \frac{d(w_j)}{\Delta w_j} + \frac{1}{2} \frac{d(w_j)}{\Delta w_{j-1}} \right)
\end{aligned}
$$
...
$$ \frac{N_{j_{min}}^{t+1} - N_{j_{min}}^t}{\Delta t} + \frac{J_{j_{min}+1}^{t+1} - R_{dd}}{\Delta w_{j_{min}}} = -\mu(w_{j_{min}}) N_{j_{min}}^{t+1} $$
...
*   The upward diffusion term from below the boundary is omitted, so $B_{j_{min}}$ does not have the $\frac{1}{2} \frac{d(w_{j_{min}})}{\Delta w_{j_{min}-1}}$ component:
    $$ B_{j_{min}} = 1 + \Delta t \mu(w_{j_{min}}) + \frac{\Delta t}{\Delta w_{j_{min}}} \left( g(w_{j_{min}}) + \frac{1}{2} \frac{d(w_{j_{min}})}{\Delta w_{j_{min}}} \right) $$
...
$$ J_{j_{max}+1} = g(w_{j_{max}}) N_{j_{max}} - \frac{1}{2} \frac{0 - d(w_{j_{max}}) N_{j_{max}}}{\Delta w_{j_{max}}} $$
...
$$
\begin{aligned}
\tilde{A}_j &= \frac{A_j}{\Delta t} = -\frac{1}{\Delta w_j} \left( g(w_{j-1}) + \frac{1}{2} \frac{d(w_{j-1})}{\Delta w_{j-1}} \right) \\
\tilde{C}_j &= \frac{C_j}{\Delta t} = -\frac{1}{\Delta w_j} \left( \frac{1}{2} \frac{d(w_{j+1})}{\Delta w_j} \right) \\
\tilde{B}_j &= \frac{B_j - 1}{\Delta t} = \mu(w_j) + \frac{1}{\Delta w_j} \left( g(w_j) + \frac{1}{2} \frac{d(w_j)}{\Delta w_j} + \frac{1}{2} \frac{d(w_j)}{\Delta w_{j-1}} \right)
\end{aligned}
$$

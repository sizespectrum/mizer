...
So $D_{num} \approx A w^p \cdot w \delta = (A \delta) w^{p+1}$.
This corresponds to the power law form for diffusion $D(w) = K w^{p+1}$ with $K = A \delta$.
...
constant_rdd_test <- function(rdi, species_params, params, ...) {
    ...
    # If K is very small, we might have issues, but here K = A(beta-1) > 0.
    a_q <- K_loc
    b_q <- -(2*A_test - K_loc)
    c_q <- -2*B_test
    det <- b_q^2 - 4 * a_q * c_q

...
weight $cw_\infty,$ where $c$ is a positive rescaling factor. Then Equation~\@ref(eq:MvF) 
...
* (Equation~\@ref(eq:mu_b)) Beyond the upper limit of body weight. 
 ...
 invariant pattern. 
mortality, given by (Equation~\@ref(eq:mup)), where the abundance $N$ is the abundance at our steady state.  This is done to ensure that the total mortality rate on explicitly modeled species is $\mu_0 w^{n-1}$ which scales with weight in the way required for scale invariance.
...
\begin{equation} M_i (w)=
   \begin{cases}
      1, & \text{if}\ w<w_{*i} \\
      \frac{1}{\left(1-\left(\frac{w}{w_{\infty.i}}\right)^{1-n}\right)}\left(\frac{1-\left(\frac{w}{w_{\infty.i}}\right)^{1-n}}{1-\left(\frac{w_{*i}}{w_{\infty.i}}\right)^{1-n}}\right)^{\frac{1}{1-n}\left(\frac{\mu_{0}}{\hbar}\right)}, & \text{otherwise}
    \end{cases}
\end{equation}
and 
\begin{equation}

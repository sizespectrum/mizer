...
For example, to set up a model using the `newMultispeciesParams()` function, the user only *needs* to provide the von Bertalanffy asymptotic sizes ($w_{\infty}$) for the species in the community. Almost all other physiological, ecological, and mortality parameters can be automatically filled in using defaults. (See the section on [maximum-size parameters](#maximum-size-parameters) below for the distinction between $w_{\infty}$, $w_{\text{repro\_max}}$ and $w_{\text{max}}$.)
...
$w_{\text{mat}} < w_{\infty} \approx w_{\text{repro\_max}} \le w_{\text{max}}$.

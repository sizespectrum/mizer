...
\begin{itemize}
    ...
    \item Running a simulation. This is done by calling the \code{project()} method with the model parameters.
\end{itemize}
...
Pass in the species parameters data.frame
Pass in the interaction
Pass in other model environment parameters: n, p, q, r\_pp, kappa, lambda, w\_pp\_cutoff
Pass in model structure parameters: w ranges

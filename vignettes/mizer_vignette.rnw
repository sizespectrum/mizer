...
Abundance of individuals decreases with body size leading to a
'size spectrum'.
Big things eat little things.

References to:
Sheldon and Parsons (1967); Andersen and Beyer (2006);
Andersen and Pedersen (2009); Blanchard; NORTH SEA PAPER etc.
...
are overwritten by the species names of the species parameters data.frame
...
This file can be opened with most spreadsheets for you to inspect.
...
You can see that the gears in the mew \class{MizerParams} object have
...
SOMETHING ABOUT THE IMPACT THAT DT HAS ON THE SIMULATION.
...
This value is then
used as the fishing by all of the gears at each time step of the projection,
...
use a two-dimensional array of values, set up as time step by gear.
...
abundances of each species at size (the sizes match those in the species
...
Like the \class{MizerParams} class this has various slots to contain the
...
We will use the $params$ object with four gears that was created in the
...
Now the model time step is 0.5 and we store the output every at multiples 
of time = 0.5.
...
In our example $params_gears$ object we have 
...
When running \code{project()} with an effort array we do not specify
the $t\_max$ argument. The duration of the projection is given by $dt$
and the number of rows in the effort array.
...
In the previous sections we have seen how to set up a model and project it
...
As you can see, \code{getSSB()} calculates the SSB though time (depending
on the argument $t_save$) for each species.
...
plotting easier. PERHAPS INCLUDE EXAMPLE OF USING GGPLOT WITH MIZER BELOW?
...
\section{A more detailed example using the North Sea}
...
We use data from the North Sea from 1957 to 2010.

As before we load the species and interaction parameters for the North Sea.
Note that we are the using dataset without fishing gears. In this example,
we want the species to be caught independently of each other:

<<label = load_ns_species_params>>=
# REMOVE CATCHABILITY FROM BOTH DATASETS
...
reads the data in as a \class{data.frame}. We ultimately want this to
...
the same as that of the species parameters.

As mentioned above, fishing mortality is calculated as a combination of
...
effort that will result in the historical fishing mortality levels.
...
This means that a relative effort of 1 will result
in the fishing mortality being what it was in 1990.

The catchability can be set by either adding an extra column
to the species parameters data.frame, or by creating a \class{MizerParams}
object and setting the catchability directly.
Here we use the former method, and then check that catchability has been
set correctly by inspecting the diagonal of the catchability slot.

HOW IS R0 BEING SET?!?!

<<label=set_catchability>>=
# Set the catchability of each of these species to 1990 level
species_params_nogears$catchability <- as.numeric(f_history["1990",])
params <- MizerParams(species_params_nogears, inter)
diag(params@catchability)
@

Now we can make the matrix of relative effort through time to be used with the
\code{project()} method.
To do this we want rescale the \code{f\_history} object to 1990 so that
the relative effort in 1990 = 1.
We then check a few rows of the effort matrix to see what has happened:
...
the population dynamocs in the early years will be strongly
determined by the initial population abundances (known as the transient
behaviour - essentially behaviour before the long term dynamics are reached).
As this is fisheries science, we don't know what the initial abundance are.
One way around is this to project forward at the first year of historical
relative effort level until equilibrium is reached.
...
We now have our matrix of efforts relative to 1990.
...
% Order of rows and columns in interaction matrix is important.
% The dimnames are not used so the order must match the order
% of the species names in the species_params data.frame.

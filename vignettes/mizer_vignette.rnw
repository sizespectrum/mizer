...
Just checking Bibtex works in vignette~\citep{andersen_damped_2010}.
...
See the help page of the plot methods for more details.
...
the final time step.
Here we plot the abundance spectra, averaged over time = 5 to 10
...
Run to equib at constant earliest fishing mortality on each species.
Then historical fishing mortality on each species (same as the 
North Sea paper).

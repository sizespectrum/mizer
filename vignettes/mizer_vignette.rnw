...
%Yes, an Rnw file inside pkgName/inst/doc will be compiled and the pdf available with vignette() You might need to declare the vignette somwhere, maybe DESCRIPTION, I can check
...
Check equation notation - subscripts etc
...
f_i(w) = \frac{E_i(w)}{E_i(w) + I_{max}}
\end{equation}

\begin{equation}
\label{eqtn:maturation}
\psi(w) = \left[1 + \left(\frac{w}{w^*_i} \right)^{-10}\right]^{-1} \left( \frac{w}{W_i}\right)^{1-n}
\end{equation}

\begin{equation}
\label{eqtn:somatic_growth}
g_i(w) = (\alpha f_i(w) h_i w^n - k_i w^p)(1 - \psi(w))
\end{equation}

\begin{equation}
\label{eqtn:gonadal_growth}
g_r(w) = (\alpha f_i(w) h_i w^n - k_i w^p) \psi(w)
\end{equation}

\begin{equation}
\label{eqtn:di_recruitment}
\varepsilon / (2w_0N_i(w_0)g(w_0))\int_{w_i^*}^{w_i}N_i(w)g_{r}(w)dw
\end{equation}
...
\mu_{p,i}(w) = \sum_j \int_{w_0}^\infty \phi \left(\frac{w^{'}}{w} \right) (1 - f_j(w^{'})) V(w^{'}) \theta_{i,j} N_j(w^{'})dw^{'}
\end{equation}
...
Initial work done~\citep{andersen_damped_2010}.
...
\begin{itemize}
    ...
    \item Running a simulation. This is done by calling the \code{project()} method on the model parameters.
...
This constructor method can take many arguments. However, creation is simplified 
...
Although many of the arguments used when creaing a \class{MizerParams} 
object are optional, there is one argument that must be supplied by
the user: the species specific parameters. 
For each species in the model community you must supply the life-history
parameters, the selectivity function parameters and the stock-recruitment
function parameters.

The species parameters must all be stored in a single data.frame object.
Each column of the parameter data.frame is a parameter and 
each row has the parameters for one of the species in the model.
...
include them in the species parameter data.frame, they will be automatically added
when the \class{MizerParams} object is created. 
A description of the columns of the species parameter data.frame
and any default values can be seen Table~\ref{tab:species_params}.
...
Fishing effort is set when the simulation is run and can vary through time
(see Section~\ref{sec:projection}).
...
the name of the species, i.e. each species is fished by a different gear.
...
relationship, based on some measure of the reproductive potential of the stock,
such as spawning stock biomass.
...
do not feed on each other) to 1 (species overlap perfectly).
If all the values in the interaction are set to 1 then predator-prey
...
contructor the default interation matrix is used. This all values set to 1. 
...
values, many of the values come from a particular model for the
...
\begin{description}
    ...
    \item[inter] The interaction matrix (see Section~\ref{sec:interaction_matrix}).
...
Table~\ref{tab:species_params}.

There is no $sel_func$ column to determine the selectivity function.
...
you enter (the results are not shown here as the array is pretty huge):
...
Here, we look at an example where we we do specify the fishing gears.
We take the original $params_data$ object that was read in and
bind an additional column, $gear$ to it. This $gear$ column will
contain the name of the gear that catches the species in that row.
...
params_gear <- MizerParams(params_data, interaction = inter)
@

You can see that the gears in the \class{MizerParams} object have
been set correctly by examining the $catchability$ slot:
<<>>=
params_gear@catchability
@

Note that we are still using the default $sigmoid\_length$
selectivity function for all the gears in this example.
...
scenarios of changing the efforts of fishing gears rather than on
individual species.
...
$t\_max$ determines the maximum time of the simulation. Note that
this is not the same as the number of time steps in the model.
The number time steps in the model is given by $t\_max$ / $dt$.
...
Seperate from these two is $t\_save$. This sets how frequently 
\code{project()} stores the state of the projection.
$t\_save$ must therefore be a multiple of $dt$.
The default value of $t\_save$ is 1.
...
$initial\_n$ is a matrix (with dimensions species by size) that
...
Like the \class{MizerParams} class this has various slots to contain the output
of the simulations.
...
the argument $t\_save$) is stored.
The $n$ and $n\_pp$ hold the abundances of the species and the background
population at size respectively. 
The $n$ and $n\_pp$ slots have one extra row than the effort.
...
first few lines).
In this example, each species is caught by a seperate gear which has the
same name as the species:
...
Now the model time step is 0.5 and we store the output every 0.5 steps.
...
for all gears and time steps. This is because we are setting the $effort$
argument as a single numeric value.
As mentioned above we can also set the effort value for each gear seperatley
using a vector of effort values. This still keeps the efforts constant
through time.
%We can see what gears we have by inspecting the
%$gear$ column of the
%$species\_params$ slot of the $params$ object.
%
%<<>>=
%params@species_params$gear
%@

For example, if we want to switch off fishing on the pelagic species
(sprat, herring and
norway pout) and keep fishing effort on demersals at 0.5 we
can set the effort for each gear using a vector, where the
positions of elements in the vector match the position of the gears:

<<>>=
effort <- c(0, 0.5, 0, 0, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5)
@

We then call \code{project()} with this effort and inspect the resulting
$effort$ slot (we use the \code{head()} function to just show the
first few lines):

<<>>=
sim <- project(params, effort = effort, t_max = 10, dt = 1, t_save = 1)
head(sim@effort)
@
...
then the length of the simulation is effectively determined by the
number of rows in the effort array and $dt$; the argument $t\_max$ is not used.
In our example $params$ object we have 
\Sexpr{length(unique(params@species_params$gear))}
gears.

In this simple example, all fishing gears have the same effort.
This effort increases over time from 0 to 0.5 by 0.1.
First we set up a vector to store this increasing effort.
Then we set up the effort array using this vector.
The dimensions of the effort array are set using the \code{dim}
argument. The array will have as many rows as the length of the effort
vector, and as many columns as there are gears.
Each row of the array is the effort at a time step (given by the $dt$
argument).
The first few rows of the effort array are shown as an illustration.

<<>>=
effort_vec <- seq(from = 0, to = 1.0, by = 0.1) 
effort <- array(effort_vec, dim = c(length(effort_vec), 12))
head(effort)
@

When running \code{project()} with an effort array 
we do not specify the $t\_max$ argument.
The duration of the projection is given by $dt$ and the number of rows in the
effort array.
...
As you can see in th previous examples, the dimensions of the effort
...
be easier to prepare it in advance as a .csv file (perhaps using an open
source spreadsheet such as LibreOffice) and read it in, similar to how
...
Now we are able to set up a model and project it forward through time
under our desired fishing scenario.
The result of running a projection is an object of class \class{MizerSIm}.
What do we then do?
How can we explore the results of the simulation?
...
useful summary methods to pull out information object out of
...
See their help files for more details.
...
so on. However, if length is specified (minimum or maximum) then it is
...
\subsubsection{Summaries example}
...
As you can see, \code{getSSB()} calculates the SSB though time for each
species.

As mentioned above we specify the size range of some of the methods.
For example, we calculate the total biomass of each species but only
...
This is done in the same way as the methods \code{getN()} and 
...
\subsubsection{Indicators example}
...
data.frames using the command \code{melt} which is in the \pkg{reshape2}
package (this is also used by the \pkg{ggplot2} package).
It is definately worth your time getting to grips with this to make
plotting easier.

Inlcuded in \pkg{mizer} are several dedicated plots that use
\class{MizerSim} objects as inputs (see Table~\ref{tab:plots}).
These plots all return objects of type \class{ggplot} from the
\pkg{ggplot2} package.
See the help page of the plots for more details.

Some of the plots plot values by size (for example \code{plotFeedingLevel()}
and \code{plotNSpectra()}).
For these plots, the default is to plot the values by size at the final time
step of the projection.
It is possible to specify a time range to average the abundances over before
plotting the values
...
\begin{table}[ht]
	...
	plotNSpectra() & Plots the abundance spectra of each species and the background community. It is possible to specify a minimum size which is useful for truncating the plot.\\
...
the final time step.
Here we plot the abundance spectra, averaged over the last 5 time periods
(the simulation was projected until $t$ = 10; see
Figure~\ref{fig:plotNSpectra_example}):

<<eval=FALSE>>=
plotNSpectra(sim, time_range = 5:10)
@

\begin{figure}
<<label=plotNSpectra_example, fig=TRUE, echo=FALSE>>=
    p <- plotNSpectra(sim, time_range = 5:10)
    print(p)
@
\caption{An example of using the \code{plotSpectra()} method, plotting values averaged over the period $t$ = 5 to 10.}
\label{fig:plotNSpectra_example}
\end{figure}


\section{North Sea example}
\label{sec:north_sea_example}

Use the data from the North Sea paper?
...
Finlay Scott would like to thank Cefas Seedcorn Project DP266 for supporting
this work.

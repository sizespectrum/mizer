...
species $i$ such that $N_i(w)dw$ is the *number* of individuals of
...
predator of species $j$ and weight $w$ that is actually consumed, this gives:

\begin{equation}
  \label{eq:mup}
  \mu_{p.i}(w_p) = \sum_j \theta_{ji} \int \phi_j(w,w_p) (1-f_j(w))
...
\begin{equation}
  \label{eq:mupp}
  \mu_{p}(w_p) = \sum_j \int \phi_j(w,w_p) (1-f_j(w)) 
  \text{search_vol}_j(w) N_j(w) \, dw.
\end{equation}

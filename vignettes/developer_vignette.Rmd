...
\end{equation}
The search volume $\gamma_i(w)$ is stored in an array `params@search_vol[i,a]`
...
# MizerParams setup functions

The parameters specifying a mizer size-spectrum model are stored in the slots
of an object of class MizerParams. Mizer provides several setup functions
to create such a MizerParams object. They are described in this section.

In this section again we are assuming that the MizerParams object holding
the model parameters is called `params`.

## emptyParams()
The `emptyParams()` function sets up a MizerParams object in which most slots
are left empty. Only the size grids as well as the species names and 
gear names are set up.
...
\begin{equation}
  ...
  \exp \left[ \frac{-(\ln(x / \beta_i)))^2}{2\sigma_i^2} \right]
...
is used for reproduction is given by

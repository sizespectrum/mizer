...
the past, using the `expect_known_value()` test. The past values are stored in
...
Some tests check that code produces the same numerical values as in the
past with the `expect_known_value()` function. This is set up to store
the correct values in files in `tests/testthat/values`.

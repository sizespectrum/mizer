...
used to fuel the needs for standard metabolism `std_metab` and 
activity and movement `activity`. The remaining rate, if any,
\begin{equation}
  \label{eq:Er}
  E_{r.i}(w) = \max(0, \alpha f_i(w) h_i w^n - \text{std_metab_i(w)} 
  - \text{activity_i(w)})
\end{equation}

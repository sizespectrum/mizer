...
[`setFishing()`](../reference/setFishing.html) recomputes the fishing setup.
Assigning `gear_params(params) <-` already triggers this, so you mainly call
`setFishing()` when supplying a `selectivity` or `catchability` **array**
directly, setting a baseline effort, or rebuilding from scratch.

...
## Introduction
As mentioned above,the trait-based size spectrum model can be derived as a simplification of the general model outline in [the model description section.](model_description.html)
...
In the trait-based model, the egg production is modeled using a *Beverton-Holt* type function (the default in `mizer`, see [the section on stock-recruitment relationships](model_description.html#sec:recruitment)) where the recruitment flux $R_i$ (numbers per time) approaches a maximum recruitment as the egg production increases.

...
%----------------------------------------------------------------------------------
# Size spectrum modelling - concepts, processes and assumptions
 {#sec:principles}
...
Roughly speaking there are three versions of the size spectrum modelling framework of increasing complexity: The *community* size spectrum model
\citep{benoit_continuous_2004, maury_modeling_2007,blanchard_how_2009,law_size-spectra_2009}, the *trait-based* size spectrum model \citep{andersen_asymptotic_2006, andersen_damped_2010}, and the *multispecies* spectrum model \citep{hartvig_food_2011-1}.
The community and trait-based models can be considered as simplifications of the multispecies model.
...
The available food (mass per volume) for a predator of weight $w$ is determined by integrating over all species and the resource weighted by the size selection function (Equation~\@ref(eq:4)): 
...
## Consumption
The encountered food is consumed subjected to a standard Holling functional response type II to represent satiation.  This determines the *feeding level*, $f_i(w)$, which is a dimensionless number between 0 (no food) and 1 (fully satiated) so that $1-f_i(w)$ is the proportion of the encountered food that is consumed. It is given by
...
The form of the allocation function (\@ref(eq:psi)) is chosen such that the growth curve approximates a von Bertalanffy growth curve if the feeding level is constant (see Figure~\@ref(fig:growth) and \citet{hartvig_food_2011-1} for details about the derivation). The actual emerging growth curves from the model will depend on the feeding level, and may even result in stunted growth curves if the feeding level drops below the *critical feeding level* where the assimilated food is just enough to satisfy standard metabolism:
...
%------------------------------------------------------
# Implementing a community-type model
 {#sec:comm_model}

## Introduction
 {#sec:community_model_intro}
...
In the summary you can see that the size range of the community spectrum has been set from \Sexpr{min(params@w)} to \Sexpr{max(params@w)} and these are spread over \Sexpr{length(params@w)} size bins. Similar information is available for the resource spectrum.
...
The resource and community spectra are shown in the bottom panel of the plot (the plotted resource spectrum has been truncated to make for a better plot, but really extends all the way back to \Sexpr{signif(params@w_full[1],3)} g).
...
## Example of a trophic cascade with the community model
 {#sec:trophic_cascade_comm_model}
...
```{r label=plot_comm_fmort, eval=FALSE}
plot(sim1)
```
...
```{r label=plot_relative_comm_abund, eval=FALSE}
...
```{r label=plot_comm_biomass_sigma1, eval=FALSE}
...
%-----------------------------------------------------------------------
# Implementing a trait-based model
 {#sec:trait_based_model}
...
## Example of a trophic cascade with the trait-based model
 {#sec:trophic_cascade_trait_model}
...
```{r label=plot_trait_fmort, eval=FALSE}
plot(sim1)
```
...
```{r label=plot_relative_comm_abund2, eval=FALSE}
...
```{r label=plot_relative_trait_m2, eval=FALSE}
...
```{r label=plot_multi_gear_trait_single_effort, eval=FALSE}
plot(sim_multi_gear)
```
...
```{r label=plot_multi_gear_trait, eval=FALSE}
plot(sim_multi_gear)
```
...
```{r label=plot_relative_comm_abund_industrial, eval=FALSE}
plot(x=sim0@params@w, y=relative_abundance, log="xy", type="n", xlab = "Size (g)",
    ylab="Relative abundance", ylim = c(0.1,10))
lines(x=sim0@params@w, y=relative_abundance)
lines(x=c(min(sim0@params@w),max(sim0@params@w)), y=c(1,1),lty=2)
...
## The species parameters
 {#sec:species_parameters_dataframe}
...
## The other \code{MizerParams() arguments}
 {#sec:other_mizerparams_arguments}
...
\begin{table}[ht]
	...
	min\_w\_pp & The smallest size of the plankton size spectrum. & 1e-10 \\
	no\_w\_pp & The number of size bins in the plankton size spectrum. & round(no\_w) * 0.3 \\
	n & The scaling of intake. & 2/3 \\
	p & The scaling of standard metabolism. & 0.7 \\
	q & The search volume exponent. & 0.8 \\
	r\_pp & The growth rate of the primary productivity (the plankton spectrum). & 10 \\
	kappa & The carrying capacity of the plankton spectrum. & 1e11 \\
	lambda & The exponent of the spectrum. & 2+q-n \\
	w\_pp\_cutoff & The cut off size of the plankton spectrum. & 10 \\
...
## Examples of making a \class{MizerParams objects}
 {#sec:params_example}
...
## Setting the initial population abundance
 {#sec:setting_initial_abundances}

When running a simulation with the \code{project()} method, the initial populations of the species and the plankton spectrum need to be specified. These are passed to \code{project()} as the arguments \args{initial\_n} and \args{initial\_n\_pp} respectively.
...
The default value for \args{initial\_n\_pp} is the carrying capacity of the plankton spectrum, stored in the \args{cc\_pp} slot of the
\class{MizerParams} parameters object.

## What do you get from running \code{project()?}
 {#sec:mizersim}
...
The \args{n} and \args{n\_pp} slots hold the saved abundances of the species and the plankton population at size respectively. 

## Projection examples
 {#sec:projection_examples}
...
```{r label=plot_basic_ms_sim, eval=FALSE}
plot(sim)
```
...
### Setting constant effort for different gears
 {#sec:proj_diff_gears_constant_effort}
...
We have seen in previous sections that a \class{MizerSim} object is made up of various ``slots'' that contain the original parameters (\args{species\_params}), the population abundances (\args{n}), the abundance of the resource spectrum (\args{n\_pp}) and the fishing effort (\args{effort}).
...
## Summary methods for \class{MizerSim objects}
 {#sec:summaries}
...
### Examples of using the summary methods
 {#sec:summaries_example}
...
```{r label=plotBiomass, eval=FALSE}
plotBiomass(sim)
```

\begin{figure}
```{r label=print_plotBiomass, echo=FALSE, fig.height=4, fig.keep='last'}
```{r plotBiomass}
```
...
```{r label=plotSpectra_example,eval=FALSE}
...
```{r label=demo_summary_plot,eval=FALSE}
plot(sim)
```
...
%------------------------------------------------------------------------------------
# A multispecies model of the North Sea
 {#sec:north_sea_example}
...
```{r label = plot_ns_biomass, eval=FALSE}
plotBiomass(sim)
```
...
%
%```{r label = plot_ns_biomass_no_trans, eval=FALSE}
%plotBiomass(sim, start_time = 1967)
%```
%
%\begin{figure}
%```{r label=print_plot_ns_biomass_no_trans, fig=TRUE, echo=FALSE, height = 4}
%```{r plot_ns_biomass_no_trans}
%```
%\caption{Simulated biomasses of stocks in the North Sea without the transients.}
% {#fig:plot_ns_biomass_no_trans}
%\end{figure}
...
```{r label=plot_ns_indicators, eval = FALSE}
library(ggplot2)
# Simulated data
...
```{r label=projected_ssb_ns, eval=FALSE}
library(reshape2)
years <- 1967:2050
...
%# Under the hood of \pkg{mizer: notes for developers}
%
%Still to come.
%
%Classes and whatnot
...
# Scale-invariant, trait-based model
 {#sec:scale_invariant_trait_based_model}
...
```{r plot_scale_sense2, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r label=demo_trait_model_insensitive, eval=FALSE, warning=FALSE, message=FALSE}
params_trait <- set_trait_model(no_sp = 10, min_w_inf = 10, max_w_inf = 1e3,
                          knife_edge_size = 100)
sim_trait_initial <- project(params_trait, t_max=100, effort = 0)
sim_trait <- project(params_trait,
...
```{r label=demo_scale_model_sensitive, eval=FALSE, warning=FALSE, message=FALSE}
params <- set_scaling_model(no_sp = 10, min_w_inf = 10, max_w_inf = 1e3,
                            min_egg = 1e-4, min_w_mat = 10^(0.4), rfac = Inf,
                            knife_edge_size = 100,kappa = 0.005)
sim <- project(params, t_max=15, effort = 1)
...
%We avoid the problem of having to model arbitrarily small species by replacing them with the plankton resource, so the 
...
## Details of changes to the trait-based model
 {#subsec:bullets}
...
# Implementing explicit species in a scale-invariant background
 {#sec:explicit}
...
```{r plot_mullet_hake_spectra, eval=FALSE}
...
```{r plot_mullet_hake_biomass, eval=FALSE}
...
```{r plot_mullet_hake_biomass_steady, eval=FALSE}

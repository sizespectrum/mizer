...
`given_species_params()` rather than with `species_params()`. The
difference is that `given_species_params()` holds only the values that
*you* have supplied, and changing one of them triggers mizer to
recalculate any other parameters that are derived from it. So this is the
safe way to change input parameters.

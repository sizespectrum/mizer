...
The mizer package can simulate the size-specrum dynamics. Given the
...
Let us start by looking at an example. Start RStudio and load the
following packages:
...
organisms's body size, and the power is about $p = 3/4$.
...
> instead of `"w"`. Then fit a linear model and thus determine that the
> mortality rate is $$\mu(w) = \mu_0\ w^{p-1} = `r m0` \ w^{-1/4}.$$
...
spectrum also follows a power law: $$N(w) = N_0\ w^{-\mu_0/g_0 - p}$$
...
> model with the changed maturity curve. The function to 
...
This exercise gives you a different set of predation kernel parameters
to investigate.
...
chose the search volume exponent automtically according to this formula.
...
After having turned on fishing, we need to find the new steady state
with `steady()` and can then plot it.

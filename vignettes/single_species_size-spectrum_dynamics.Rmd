...
The `biomass = FALSE` argument to the `plotSpectra()` function specifies
that we want to plot the number density, rather than the biomass density
that is plotted by default.
...
argument `per_log_size = TRUE` to `plotSpectra()`.
...
`reproduction_level(params) <- 0`. This
...
Setting `reproduction_level` on the changed model retunes it to the value
...
$$q = \lambda -2 + n.$$ This explains the message you got when you

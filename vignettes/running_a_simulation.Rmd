...
of the species and the plankton spectrum need to be specified. These are passed
to `project()` as the arguments `initial_n` and `initial_n_pp` respectively.
`initial_n` is a matrix (with dimensions species x size) that contains the
initial abundances of each species at size (the sizes must match those in the
species size spectrum). `initial_n_pp` is a vector of the same length as the the
length of the full spectrum.

Default values for the initial values are automatically set when creating a
MizerParams object. So you can call `project()` without specifying these
arguments.
...
be seen in the range of predation moratlity and feeding level values. With the
...
the simulation results returned by `N()` and `NPlankton()`. Here we reduce `t_save`

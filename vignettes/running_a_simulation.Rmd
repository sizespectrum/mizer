...
Running `project()` returns an object of type `MizerSim` that stores the results
of the projection. These can be accessed with various functions. The function
`params()` returns the `MizerParams` object that was passed in to `project()`.
The function `effort()` returns the fishing effort of each gear through time.
...
`t_save`). The `n()` and `n_pp()` functions return arrays with the saved
abundances of the species and the plankton population at size respectively.
Finally the `times()` function returns the vector of times at which simulation
results were stored.

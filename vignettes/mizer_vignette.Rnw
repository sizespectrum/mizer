...
\label{sec:size_based_assumptions}
...
THOUGHT THAT TRAIT BASED USED HOCKEY STICK
...
The \code{set\_community\_model()} function is called by passing in the arguments by name. Any parameter that is not passed in is set to the default value:
...
If you reading this vignette for the first time, it is likely that you have no idea what a \class{MizerParams} object actually is. In 'R-speak' it is an \code{S4} object, which means it is made up of 'slots'. Slots are essentially containers that store the object data. These slots can be seen by running:

<<label=slots_MP>>=
slotNames(params)
@

As you can see, the \code{params} object is made up of lots of slots. They are discussed in detail in Section~\ref{sec:MizerParams}. These slots are accessed by using the \code{@} operator. For example, to access the \code{w} slot (which contains a vector of the size bins in the model) you would use:
...
We want to project this new model through time using the \code{project()} method. However, note here that we are projecting the model by 100 time steps. More importantly note that we have introduced a new argument: $dt$. This is the step size of the solver. It does not have anything to do with the biology in the model. It only affects the internal engine of \code{project()} that performs the projection. 
As you can see in the underlying model equations in Section~\ref{sec:size_based_assumptions}, the model runs in continuous time. Therefore, to project it forward, \code{project()} must solve the system of equations using approximations. The quality of these approximations is strongly affected by $dt$. 
...
sim_sigma1 <- project(params_sigma1, effort = 0, t_max = 100, dt=0.01)
@
...
The main parameters of interest are the number of the species in the model (\code{no\_sp}) and the minimum and maximum asymptotic sizes (\code{min\_w\_inf} and \code{max\_w\_inf} respectively). Here we set up the model to have 10 species, with asymptotic sizes ranging from 4g to 100 kg (the sizes are spread evenly on a logarithmic scale).
...
In the community model, recruitment is constant and there is no relationship between the abundance in the community and the recruitment.
In the trait-based model, the SRR is modeled using a ''hockey-stick'' type function, where the realised numbers entering the smallest size group are linearly related to the density-independent egg production (Equation \ref{eq:Rp}), but capped my a maximum value (for more details see Appendix 1 in \citep{andersen_damped_2010}).
This maximum value is controlled by a scalar parameter, $k0$. This should be adjusted by the user so that the community spectra roughly forms a continuation of the resource spectrum. The default value of $k0$ is 100.
For further information on how SRRs are dealt with in \pkg{mizer} see Section \ref{sec:srr}.

CAREFUL - RECRUITMENT SECTION MENTIONS BH SRR
ALSO CHECK A and P 2010 to see if SRR is implemented properly in wrapper function

<<label=demo_trait_model_params>>=
params <- set_trait_model(no_sp = 10, min_w_inf = 4, max_w_inf = 1e6, k0 = 100)
@
...
to \Sexpr{max(params@species_params$w_inf)}. Each species has the same preferred predator-prey mass ratio parameter values ($beta$ and $sigma$, see Equation \ref{eq4}). There are \Sexpr{length(params@w)} size bins in the community.
...
Here we project the model for 50 time steps and without any fishing (the $effort$ argument is set to 0). We use the default initial population abundances given by the \code{get\_initial\_n()} function (see Section \ref{sec:setting_initial_abundances}).

<<label=trait_project_no_fishing>>=
sim <- project(params, t_max=50, effort = 0)
@

This results in a \class{MizerSim} object which contains the original \class{MizerParams} object, as well as the abundances of the spcecies and the background spectra through time.
...
<<plot_comm_sim>>
@
\caption{Example plot of the trait-based model}
...
The default fishing selectivity function is a knife-edge function, which only selects individuals larger than 1 kg. There is also only one fishing gear in operation, and this selects all of the species. You can see this if you call the \code{summary()} method on the $params$ argument we set up above. At the bottom of the summary there is a section on ''Fishing gear details''. You can see that the there is only gear, called ''knife\_edge\_gear'' and that it selects species 1 to 10.

It is possible to include more than one fishing gear in the model and for different species to be caught by different gears. It is also possible for a species to be caught by more than one fishing gear. We will ignore this now, but will explore it further below. 

We set up the \class{MizerParams} object in the same way as we did before. The $knife\_edge\_size$ parameter is explicitly passed in for clarity.

<<label=set_trait_fishing>>=
params_knife <- set_trait_model(no_sp = 10, min_w_inf = 4, max_w_inf = 1e6, k0 = 100, knife_edge_size = 1000)
#params_knife@species_params$erepro[] <- 0.01
@
...
Now we simulate with fishing. Here, we use an effort of 1. As mentioned earlier, the fishing mortality on a species is the product of effort, catchability and selectivity (see Section~\ref{sec:fishing_gear} for more details). Selectivity ranges between 0 and 1. By default catchability is 1. Therfore, in this simulation the maximum fishing mortality is given by the effort. This effort is constant throughout the duration of the simulation (however, this is not necessarily the case, see Section\ref{sec:projection}).

<<label=sim_trait_with_fish>>=
sim1 <- project(params_knife, effort = 0.5, t_max = 50)
@

%This can be plotted using the \code{plotFMort()} method .
By plotting a summary of fished community, the knife-edge selectivity at 1000g can be clearly seen (Figure \ref{fig:plot_trait_fmort}):

<<label=plot_trait_fmort, eval=FALSE>>=
plot(sim1)
@

SOMETHING IS GOING EXTINCT!
...
The trophic cascade can be explored by looking at the change in total abundances of all species at size when the community is fished and unfished.
...
As with the community model, we are interested in the relative total abundances by size in the final time step. We can use the \code{apply()} function to help us:
...
We can use these vectors to calculate the relative abundances:

...
\begin{figure}
{\bf fishing comparison plot here}
%<<label=print_plot_trait_ins, echo=FALSE, fig.height=4, fig.keep='last'>>=
%<<plot_trait_ins>>
%@
\caption{Biomass plots following introduction of fishing (a) in the trait-based 
model from Section {\bf(reference to Section 6)}, and (b) in the scale-invariant 
model developed here.  There are ten species; $W$ is taken over a range 10 g
to 100 kg, uniformly spaced on the log-scaled mass axis. The 
initial state is close to the steady state in the absence of fishing.  
Recruitment to the multispecies fishery is knife-edge at 1 kg for all 
species with $W > 1$ kg, and fishing mortality rate is set 
at $F = 0.75$ yr$^{-1}$.}
...
more sensitive to fishing (Figure~\ref{fig:plot_biomass_tb_vs_si}b).  
...
internal dynamics. {\bf (Rossberg)  Please tell me if you think I'm wrong 
about the emerging SR.}
You can construct Figure~\ref{fig:plot_biomass_tb_vs_si}b, using the 
following code {\bf(currently commented out)}:
...
to be near, after aggregating over species {\bf(Sheldon)}.  The significance 
...
model {\bf(Cuesta et al)}.  Related numerical simulations using  finite
number of species support the proposition that coexistence of multiple 
species is easier to achieve near the power law {\bf (Law et al arxiv paper)}.  
These properties readily extend to multicellular fish assemblages 
{\bf (Southwell, Delius unpublished results)}.
...
{\bf (Gustav: can you see if this makes sense? It is hard to define this 
stuff without going in to all the details.)}  
...
{\bf (Sheldon)}.  The existence of this remarkable regularity, plus the 
...
To help set up a scale invariant trait-based model, there is a wrapper function, 
%\code{set\_scaling\_model()}. 
...
them have default values so you don't need to worry about them for the moment. 
...
One can see from the plot of time versus biomass that the system is 
...
Section~\ref{sec:explicit}, but includes some modifications and restrictions to 
...
\begin{itemize}
 ...
 \item {\bf(Ref to Eqn 3.4)}  Feeding depends on prey body size, but not on 
 ...
 \item {\bf(Ref to Eqn 3.8)} {\bf(this piece wants dropping if we don't use 
 a step-like psi)}.  We assume that the proportion of energy allocated to 
 ...
 for scale invariance;  the assumption was made for the mathemaical 
 reason that it is then possible to get a closed-form expression for 
 the steady state.

 \item {\bf(Ref to Eqn 3.11)} Scale invariance requires egg weight to 
 ...
 adult weight {\bf (Andersen et al. review)}.  Scale invariance of egg 
 ...
 \item {\bf(Ref to Eqn 3.12)}  A stock-recruitment relationship is not 
 ...
 scale-invariant trait-based model by setting the value of 
 %\args{r\_fac}. 
 Here %\args{r\_fac} 
 equals the maximum recruitment $R_{max.i}$ divided by the actual 
 ...
 \item {\bf(Ref to Eqn 3.14)}: beyond the upper limit of body weight. 
 The total mortality
...
\item {\bf(Ref to Eqn 3.15)} 
The resource spectrum extends from the smallest weight at which an explicitly 
...
params <- set_scaling_model(no_sp = 10, min_w_inf = 10, max_w_inf = 1e4,
                            min_egg = 1e-4, min_w_mat = 10^(0.4),
                            knife_edge_size = 100, kappa = 0.005)
params@A[] <- NA
@

Here \args{params@A[i]} should be set to $1$ if $i$ is a foreground species, and \args{params@A[i]} should be set to \args{NA} if $i$ is a background species.    
...
The stock recruitment relationship can be controlled by setting the value of \args{r\_fac}, in a similar way to as described in Subsection \ref{subsec:modtrait}.
...
<<plot_mullet_sim>>
@
\caption{Example plot of time vs biomass for the scale invariant trait-based model plus mullet with no fishing.}
...
<<plot_mullet_sim2>>
@
\caption{Example plot of weight vs abundance for the scale invariant trait-based model plus mullet with no fishing, in the final state of our simulation.}
...
In practical fisheries science one will have many species that one wishes to model explicitly. Further species can be added into our system by repeated use of the \code{add\_species()} command. Let us demonstrait this by adding hake into our system with background and mullet. 

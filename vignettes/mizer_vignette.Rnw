...
\title{Multispecies, trait and community size spectrum ecological modelling in \R (\pkg{mizer})}
...
It is more complicated than a community model and the most significent difference between the two is that while the community model only resolves a single ``species'', the trait-based model resolves many species.

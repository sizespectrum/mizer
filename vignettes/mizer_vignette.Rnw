...
\title{Multispecies size spectrum ecological modelling in \R (\pkg{mizer})}
...
The species in the model are \Sexpr{paste(params_data[,"species"], collapse=", ")} which account for about 90\% of the total biomass of all species sampeld by research trawl surveys in the North Sea. 
...
\subsection{Settin up and running the simulation}
...
Here we make an inital effort matrix of 100 years at the first effort level. We need to include dimension names for the time dimension.
...
Both of our new effort scenarios still include 100 years at the 1967 level to reduce the impact of the transisent behaviour.

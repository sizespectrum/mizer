...
In the trait-based model, the SRR is modeled using a ''Beverton-Holt'' type function (the default in \pkg{mizer}, see Section~\ref{sec:recruitment}) where the recruitment flux $R_i$ (numbers per time) approaches a maximum recruitment as the egg production increases.
The maximum recruitment flux is calculated using equilibrium theory and a recruitment multiplier (see \citet{andersen_damped_2010}), $k0$, which can be passed in as an argument to the \code{set\_trait\_model()} function. $k0$ has a default value of 50. 
...
Here we project the model for 75 time steps and without any fishing (the $effort$ argument is set to 0). We use the default initial population abundances given by the \code{get\_initial\_n()} function so there is no need to pass in any population values (see Section \ref{sec:setting_initial_abundances}).
...
To control the size at which individuals are selected there is a $knife\_edge\_size$ argument to the \code{set\_trait\_model()} function. This has a default value of 1 kg.
...
To set up the trait-based model to have fishing we set up the \class{MizerParams} object in exactly the same way as we did before but here the $knife\_edge\_size$ parameter is explicitly passed in for clarity:
...
\begin{figure}
<<label=print_plot_relative_trait_m2, fig=TRUE, height = 4>>=
...
In this section we only want to operate an ''industrial'' fishery, like in \citep{andersen_damped_2010}. Industrial fishing targets the small zooplanktivorous species that are typically used for fishmeal production. In the previous simulations we have had only one fishing gear and it targetted all the species in the community.

This requires us to look more closely at how fishing gears are handled with \pkg{mizer}.
In the previous trait-based model examples we have only considered a single fishing gear that catches all species. This gear had a knife-edge selectivity that only selected species larger than 1 kg.
You saw this earlier when you called \code{summary()} on the \code{params} object created above.
In \pkg{mizer} it is possible for a fishing gear to catch only a subset of the species in the model, or even for each species to be caught by a seperate gear.
This is useful because when projecting forward with \code{project()} you can specify the effort per gear.
 
Here we implement an industrial type fishery. The industrial fishery will only target species that have an asymptotic size less than or equal to 500g.
We will still use a knife-edge selectivity and the knife-edge will occur at 0.05 x the asymptotic size i.e. the selectivity pattern will be different for each species, depending on the asymptotic size.
This means that we must set up the model so that each species is fished by a different fishing gear, and that fishing gears will have different selectivities given by the asymptotic size of the species.
This is straight forward to do.

In the previous examples, we passed an argument $knife\_edge\_size$ to the \code{set\_trait\_model()} function.
$knife\_edge\_size$ was passed in as a single value which was used for all the species.
Now we are going to pass $knife\_edge\_size$ as a vector with the same length as the number of species in the model. The values in the vector will be the positions of the knife-edge for each species.
To start with we need to know what the asymptotic sizes are of the species in the model. As mentioned above, the \code{set\_trait\_model()} function spaces the asymptotic sizes equally on a logarithmic scale. This means we calculate them by hand.
...
Finally, we can create our \class{MizerParams} object by passing in the $knife\_edge\_sizes$ argument:

<<label=set_trait_multiple_gears>>=
params_multi_gear <- set_trait_model(no_sp = no_sp, min_w_inf = min_w_inf, max_w_inf = max_w_inf, knife_edge_size = knife_edges)
@
...
The columns of the \code{species\_params} \class{data.frame} that we are interested in are $species$ (the name of the species, here just numerical identifiers), $w\_inf$ (the asymptotic size), $sel\_func$ (the selectivity function for that species), $knife\_edge\_size$ (the knife-edge of the selectivity) and $gear$ (the name of the fishing gear, note that here each name is unique signifying that each species is caught by a different gear).
...
In our previous examples of calling \code{project()} we have specified the fishing effort with the $effort$ argument using a single value. This fixes the fishing effort for all gears in the model, for all time steps. We can do this with our multi-gear parameter object:

<<label=project_multi_gear_trait_single_effort>>=
sim_multi_gear <- project(params_multi_gear, t_max = 75, effort = 0.5)
@

By plotting this you can see that the fishing mortality for each species now has a different selectivity pattern (Figure~\ref{fig_trait_multi_gear_single_effort}).
...
\begin{figure}
<<label=print_plot_trait_multi_gear_single_effort, fig=TRUE, echo=FALSE>>=
...
When specifying the $effort$ argument for \code{project()} there are several ways of doing it. Above we specified a single value that was used for all gears, for all time steps. It is also possible to specify a separate effort for each gear. This means that we can ''turn off'' the effort for the gears that are fishing on species with an asymptotic size greater than 500 g by setting the effort to 0. To do this we can create a vector of the gears that we want to be operational (those that fish species with an asymptotic size less than or equal to 500 g) and then multiply this by the desired effort value:

<<label = setting_effort_trait_multigear>>=
operating_gears <- w_inf <= 500
effort <- operating_gears * 0.5
@

This gives us an effort vector, the same length as the number of gears in the model:
<<label=show_trait_effort_multigear>>=
effort
@

By passing this vector into \code{project()} as the $effort$ argument we can effectively switch on and off the gears we want:

<<label=project_trait_multigear>>=
sim_multi_gear <- project(params_multi_gear, t_max = 75, effort = effort)
@

Now you can see that the fishing mortality for species larger than 500 g is 0 (Figure~\ref{fig_trait_multi_gear}).
...
\begin{figure}
<<label=print_plot_trait_multi_gear, fig=TRUE, echo=FALSE>>=
<<plot_multi_gear_trait>>
@
\caption{Summary plot for the trait-based model with multiple gears when only gears fishing on species with asymptotic size of 500 g or less are operational.}
\label{fig_trait_multi_gear}
\end{figure}

\subsection{The impact of industrial fishing}

In the previous section we set up and ran a model in which an industrial fishery was operating that only selected smaller species. What is the impact of such a fishery?
We can again compare abundances to the fished ($sim\_industrial1$) and unfished ($sim\_industrial0$) cases:

<<label = industrial_fishery_simulation>>=
sim_industrial0 <- project(params_multi_gear, t_max = 75, effort = 0)
sim_industrial1 <- project(params_multi_gear, t_max = 75, effort = effort)

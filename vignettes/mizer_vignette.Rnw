...
A range of summary methods and plots are available to make this job easier.  We will look at those in Section~\ref{sec:exploring}.
...
For example, if we want to switch off the industrial gear (i.e. effort = 0), keep the pelagic gear effort at 1, and set the effort of both the trawl gears at 0.5 we set the \args{effort} like this: 

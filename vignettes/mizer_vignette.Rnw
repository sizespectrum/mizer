...
We shall describe how to run our scale invarient model, and make some comparisons with the old trait based model, and then discuss the inner workings of our scale invariant model in more detail.

\subsection{Implementing the scale invariant model}

To help set up a scale invariant trait-based model, there is a wrapper function, \code{set_scaling_model()}. Like the \code{set\_community\_model()} and \code{set_trait_model()} functions described above, this function can take many arguments. Most of them have default values so you don't need to worry about them for the moment. See the help page for more details.

<<label=help_set_scaling_model, eval=FALSE>>=
?set_scaling_model
@

The main parameters of interest are the number of the species in the model (\args{no\_sp}), the minimum egg size (\args{min\_egg}), the minimum maturity size of a species (\args{min\_w\_mat}) and the minimum and maximum asymptotic sizes (\args{min\_w\_inf}) and (\args{max\_w\_inf}) respectively. The egg, maturity and asymptotic sizes are spread evenly on a logarithmic scale. The stock recruitment relationship is controlled by the value of (\args{r\_fac}), which is the ratio of the maximum recruitment $R_{max.i}$ and the actual recruitment $R_i$ at our steady state. Setting \args{r\_fac=Inf} corresponds to setting $R_{max.i} \rightarrow \inf,$ which corresponds to turning the stock recruitment relationship off.  

<<label=demo_scaling_model_params>>=
params <- set_scaling_model(no_sp = 11, min_w_inf = 10, max_w_inf = 10 ^ 3, min_egg = 10 ^ (-4), min_w_mat = 10 ^ (0.4), rfac = Inf)
@
...
The scale invariant trait-based model corresponds to a special case of the general size spectrum model described in Section~\ref{sec:principles}.  Some restrictions 
on the trait-based model Section~\ref{sec:explicit} do need to be put in place, and 
this leads to the following changes in parameters and functions:

\begin{itemize}
 \item In order for our model to be scale invariant we require that each pair of species interact with one another with the same strength, and so $\theta_{i,j}=1$ for each 
pair of species $i$ and $j.$
 \item In order for our model to be scale invariant we require that the exponent of metabolic intake $n$ equals the 
exponent $p$ of metabolic loss. {\bf already in old trait based model ?}
 \item In order for our model to be scale invariant we require that the exponent of the resource spectrum, the exponent of metabolic intake and the exponent of the volumetric search rate are related by the equation $\lambda = 2-n+q.$  {\bf already in old trait based model ?}
...
that the egg size of most fish is constant at around one milligram, and so our assumption of egg size being proportional to asymptotic size could be taken as an idealization for mathematical simplicity.
 \item Scale invariance allows us to construct a state which is steady without the need for a stock recruitment relationship. The degree of the stock recruitment relationship imposed is controlled by a new factor $R_{fac},$ which is defined so that $R_{max.i} = R_i R_{fac}$ for each species $i,$ where $R_i$ is given by (Equation~\ref{eq:R}), when the system is at our steady state. Dropping the stock recruitment relationship corresponds to setting $R_{fac} \rightarrow \infty.$  In this case (Equation~\ref{eq:R}) is replaced with the simple relationship $R_i = R_{p.i}.$ One should note that we have not 
been able to prove that our steady state is \emph{stable} (in the sense that states close to the steady state are attracted back to it), although we have always found it to be stable under the realistic parameter settings implemented under our simulations. If the user find the steady state to be 
unstable under their chosen settings (or in a larger system, where other species/effects are added), one may include a stock recruitment relationship by specifying a finite value of $R_{fac}$. 
\item The total mortality
 ...
 mortality.  Thus, rather then defining background mortality by (Equation~\ref{eq:mu_b}) we use the expression $ \mu_{b.i}(w) = \max \{ \mu_0 w^{n-1} - \mu_{f.i}(w) - \mu_{p.i}(w),0 \}$ where 
 \begin{equation}
...
combination of the background resource spectrum and the aggregate abundance of the explicity modeled species is the power law $\kappa w^{-\lambda}.$ This is achieved choosing the carrying capacity of the background resource to be 
...
convenient, in the sense that it allows to obtain an exact analytic expression for our steady state.
\end{itemize}

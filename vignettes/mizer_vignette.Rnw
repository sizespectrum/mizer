...
\section{Introduction to size-based modelling}
...
\begin{figure}[t]
  \centering
\includegraphics[width=0.8\textwidth]{Spectrum.eps}
  \caption{A solution of the model under the assumption of a constant feeding level $f(w) = 0.6$, showing community spectrum $N_c(w)$ (thick line) and trait class spectra $N_i(w)$ (thin lines). The dashed line is the theoretically expected community spectrum $\propto w^{-2-q+n}$ ,which is lifted up for visibility. }
  \label{fig:equ}
\end{figure}

In the trait-based model, density dependence is modelled as a compensation on the egg production such that the recruitment flux $R_i$ (numbers per time) approaches a maximum recruitment as the egg production increases.  The compensation can be modelled mathematically analogous to the Holling type II function response as a ``Beverton-Holt'' type of stock-recruitment function:

\begin{equation}
  \label{eq:R}
  R_i = R_{\max.i} \frac{R_{p.i}}{R_{p.i} + R_{\max.i}},
\end{equation}
where $R_{\max.i}$ is the maximum recruitment flux of each trait class (Fig.~\ref{fig:recruitment}). 
...
The free parameter $\kappa$ may in principle be determined analytically as was done in Fig.~\ref{fig:equ}. However, that was based on an assumption that all species had maximum recruitment, $R_i = R_{\mathrm{max}.i}$. The depletion of resources makes it difficult to find an analytical approximation of $\kappa$ and until that has been achieved, $\kappa$ has to be adjusted manually such that the resulting community spectrum forms a continuation of the resource spectrum.
...
With \pkg{mizer} is is possible to implement the three types of model described above (with increasing complexity): community, trait-based and food-web.
In this Vignette we present examples of all three types of model.

Using the package is relatively simple.  There are three main stages:

\begin{itemize}
    \item Setting the model parameters. This is done by creating an object of class \class{MizerParams}. This includes model parameters such as the life history parameters of each species, and the range of the size spectrum. It is possible to create \class{MizerParams} directly using the class constructor or by using one of the convenient wrapper functions provided in the package.
...
For convenience, a wrapper function, \code{set\_community\_model()}, has been provided that creates a \class{MizerParams} object for specifially modelling a community model. The documentation for the function can be seen by entering:

<<label=help_set_community_model, eval=FALSE>>=
?set_community_model
@

The function can take many parameters which are passed on to the \class{MizerParams} constructor. We can ignore most of these parameters for the moment as they almost all come with default values. Full details of those arguments can be seen in Section~\ref{sec:MizerParams}.
...
Although default values for these parameters are provided, the user is encouraged to explore how changing the values affects the simulated community. For example, the default value of $z0$ is 0.1. Increasing this value effectively 'shortens' the length of the community spectrum. 
The value of the recruitment should be set so that community spectrum is a continuation of the background spectrum. This can be done with trial error.
A reasonable value for \code{alpha} is 0.2 \citep{andersen_life-history_2008}. 

The \code{set\_community\_model()} function can be called like this (any parameter that is not passed in is set to the default value):
<<label=demo_comm_model_params>>=
params <- set_community_model(z = 0.2, f0 = 0.7, alpha = 0.2, recruitment = 4e7)
@

Calling the function creates and returns an object is of type \class{MizerParams}. We can check this using the \code{class()} function.
<<label=class_comm_params>>=
class(params)
@

If you reading through this for the first time, it is likely that you have no idea what an \class{MizerParams} object actually is. In 'R-speak' it is an \code{S4} object, which means it is made up of 'slots'. Slots are essentially containers that can store different types of information. These slots can be seen by running:

<<label=slots_MP>>=
slotNames(params)
@

As you can see, the \code{params} object is made up of lots of slots. These slots are accessed by using the \code{@} operator. For example, to access the \code{w} slot (which contains a vector of the size bins in the model) you would use:

<<label=demo_slot_accessor, results=hide>>=
params@w
@

Don't worry about this for the moment. I'm only telling you for your own interest.

Rather than picking through the slots to find out what is in a \class{MizerParams} object, a summary of the object can be seen by calling the \code{summary()} method on it:

<<label=summary_comm_params>>=
summary(params)
@
% Set Wmat to 0 afterwards?
You can see that the size range of the community spectrum has been set from \Sexpr{min(params@w)} to \Sexpr{max(params@w)} and these are spread over \Sexpr{length(params@w)} size bins. Similar information is available for the background resource spectrum.
The community is made up of one species which has an asymptotic size of \Sexpr{params@species_params$w_inf[1]} and a preferred predator prey mass ratio of \Sexpr{params@species_params$beta[1]}. These values have all been set by default using the \code{set\_community\_model()} function. If you want to set different values for these, you will need to call the \code{set\_community\_model()} function with the desired values. 

We now have a \class{MizerParams} object that contains all the information we need about the community. We can use this to perform a simulation and project the community through time.
In the \pkg{mizer} package, projections are performed using the \code{project()} method. You can see the help page for more detail and it is described fully in Section~\ref{sec:projection}.
We will ignore the details for the moment and use \code{project()} to run some simple projections.
The parameters we need to be concerned with are $effort$, which determines the fishing effort and therefore fishing mortality through time, and $t\_max$ which is the length of the simulation.
...
The resulting object is of type \class{MizerSim}. This class the results of the simulation, as well as the model parameters. It is explained in detail in Section~\ref{sec:projection}.
After running the projection, it is possible to explore the results using a range of plots and analyses. These are described fully in Section~\ref{sec:exploring}.

To quickly look at the results of the projection you can call the generic \code{plot()} method. This plots the feeding level, predation mortality, fishing mortality and abundance by size in the last time step of the simulation, and the biomass through time (Figure~\ref{fig:plot_comm_demo}).

\begin{figure}
<<label=print_plot_comm_sim, fig=TRUE, echo=FALSE>>=
plot(sim)
@
...
Here, of course, fishing mortality is 0 because we set the $effort$ argument to 0 when running the simulation.

\subsection{Example of a trophic cascade with the community model}

It is possible to use the community model to simulate a trophic cascade, similar to those seen in \citep{andersen_damped_2010}.
To do this we need two simulations, one with fishing and one without.
This means we need to consider the fishing selectivity function. By default, the fishing gear selectivty function is a sigmoid function based on length. It has two parameters \code{l25} and \code{l50} which are the lengths at 25 and 50\% selectivity respectively. These lengths are converted to weights using default length-weight conversion parameters of $a$ = 0.01 and $b$ = 3.0.
In this example we would like to simulate a knife-edge type of fishing selectivty, with species larger than 1 kg being selected. This means we have to calculate values for  and \code{l50}.

<<label=calc_sel_for_comm>>=
a <- 0.01
b <- 3.0
selected_weight <- 1000
l50 <- (selected_weight/a)^(1/b)
@

Setting \code{l25} to be only slightly smaller than \code{l50} approximates a knife-edge selectivity function.
We add these parameters to the wrapper function to get our \class{MizerParams} object:

<<label='set_comm_fishing'>>=
comm_params <- set_community_model(z0 = 0.1, recruitment = 4e7, alpha = 0.2, f0 = 0.7, l50 = l50, l25 = l50*0.99)
@

First we simulation without fishing in the same way we did above:

<<label=sim_comm_no_fish>>=
sim0 <- project(comm_params, effort = 0, t_max = 50)
@

Now we want to simulate again, this time with fishing. This is done by setting the fishing effort argument in \code{project()}. In the simulations, fishing mortality is calculated as the product of the fishing selectivity, effort and catchability (see Section~\ref{sec:MizerParams} for more details). By default catchability is set to 1. This means that a fishing effort of 1 will result in a fishing mortality of 1 for fully selected sizes.

Here we run a simulation with fishing turned on:

<<label=sim_comm_with_fish>>=
sim1 <- project(comm_params, effort = 1, t_max = 50)
@

You can compare the difference between these scenarios by using the \code{plot()} method as before.

Here we are interested in looking at the relative change in abundance when the community is fished. To do this we need to get the resulting abundances at size from the simulation objects. The abundances are store in the \code{n} slot of the \class{MizerSim} objects. The \code{n} slot returns a three dimensinoal array with dimensions time x species x size. Here we have 51 time steps (50 from the simulation plus one which is the initial population), 1 species and 100 sizes:
...
This can then be plotted (Figure~\ref{fig:plot_relative_comm_abund}). The impact of fishing on species larger than 1000g can be clearly seen.
As described in \citep{andersen_damped_2010}, the fishing pressure lowers the abundance of large fish. This then relieves the predation pressure on their smaller prey, leading to an increase in their abundance. This in turn increases the predation mortality on their smaller prey, which reduces their abundance and so on.

\begin{figure}
<<label=plot_relative_comm_abund, fig=TRUE>>=
...
The impact can also be seen in the predation mortality. This can be seen using the \code{getM2()} method for \class{MizerSim} objects (see the help page for more details). Only a selected size range is plotted to illustrate the impact (Figure ~\ref{fig:plot_m2_comm}).
...
\section{Setting up and running a community type model}
\label{sec:community_example}

It is possible to use \pkg{Mizer} to implement a simple community model as described in
Section~\ref{sec:community_model_intro}.
By using the wrapper function \code{set\_community\_model()} it is possible to create 
a \class{MizerParams} object that sets up a community model.
This can be projected through time in the ususal way using the \code{project()} method.
The community model has the following features that distinguish it from the food web models described earlier:
\begin{itemize}
    \item There is only one 'species' in the community.
    \item The recruitment function has been set to give constant recruitment.
    \item The \code{psi} slot is set to 0 so that no energy is used for reproduction.
    \item Standard metabolism has been turned off (the parameter \code{ks} has been set to 0).
    \item The growth rate is a product of the ingested food and the assimilation efficiency.
    \item The resource spectrum extends only up to the start of the community spectrum.
    \item The \code{gamma} species parameter is calculated using equation~\ref{eq:gamma} and the argument \code{f0}.
\end{itemize}

The arguments to \code{set\_community\_model()} are described in the help page. Many
of the arguments are the same as those for the \class{MizerParams} constructor.
The two parameters which are new are \code{f0} and \code{recruitment}.
\code{f0} is the initial feeding level (described in Section~\ref{sec:parameters}) and used to calculate $\gamma$.
\code{recruitment} is the level of constant recruitment.

Although \code{set\_community\_model()} has many parameters, the dynamics of the community can be broadly controlled through the parameters
\code{recruitment}, \code{f0}, \code{alpha} and \code{z0}. 
As a guideline, \code{recruitment} should be set so that the community spectrum continues the background spectrum
and \code{z0} should be adjusted to get a reasonable spectrum.

%SOMETHING ON CYCLIC BEHAVIOUR?

For example, a community can be set up as:

<<label=set_up_community_model>>=
comm_params <- set_community_model(z0 = 0.2, recruitment = 3e7, alpha = 0.2, f0 = 0.7)
@

This can then be projected through time in the ususal way. It may be necessary to reduce \code{dt} (for example, to 0.1) to reduce any instabilities with the solver.

For example, to project it forward without any fishing pressure:

<<label=project_community_model>>=
comm_sim <- project(comm_params, effort = 0, t_max = 50, dt = 0.1)
@

The resulting abundance spectra can be seen in Figure~\ref{fig:plotSpectra_community_example}

\begin{figure}
<<label=plotSpectra_community_model, fig=TRUE, echo=FALSE>>=
p <- plotSpectra(comm_sim, biomass=FALSE)
print(p)
@
    \caption{An example background and community abundance spectra of a simple community model created using the \code{set\_community\_model()} function.}
\label{fig:plotSpectra_community_example}
\end{figure}

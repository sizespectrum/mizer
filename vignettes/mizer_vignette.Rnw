...
\affil[\dag]{Maritime Affairs Unit, IPSC, European Commission Joint Research Centre, IT}
...
that the \args{params\_data} object is a data.frame with columns \Sexpr{paste(colnames(params_data),collapse=", ")}.
...
%Note that the order of the species in the \code{f\_history} object is the same as that in the species parameters data.frame.
...
Here we make an inital effort matrix of 100 years at the first effort level, and then stick it on top of the original matrix of historical relative effort:

<<label=add_transient_years>>=
initial_effort <- matrix(relative_effort[1,],byrow=TRUE, nrow=100,ncol=ncol(relative_effort))

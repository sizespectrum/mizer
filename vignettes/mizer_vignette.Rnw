...
Calculate biodiversity target  = 0.1 * B\_F0 (use SSB)
Community target = 0.8 LFI\_F0 (= 0.32, so is the target in the paper the 'official' one or one we came up with)

Growth curves... (S2)

Compare outputs to ICES data.
SSB time series (Figure 3 in NS paper) - compare to biodiversity targets 
Yield time series

Look at change in slope and other metrics over time (Figure 4 in NS paper see page 12 for notes)
Only sizes 10g to 100 kg (?) were used:w
Only demersal species included
Compare to food web indicators: LFI (> 40cm)


New section. Get FMSY values Table 1
Project to 2050 under two scenarios:
1) F = F2010
2) F2010 to F2015 = linear change to FMSY, 2015 to 2050 at FMSY
Not done under stochasticity (here)
Compare yields under scenarios and whether they hit biodiv and community targets

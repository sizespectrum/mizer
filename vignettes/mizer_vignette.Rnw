...
comm_params <- set_community_model(z0 = 1.0, recruitment = 1e7, alpha = 0.2, f0 = 0.4)
@

This can then be projected through time in the ususal way. For example, to project it forward without any fishing pressure:

<<label=project_community_model>>=
comm_sim <- project(comm_params, effort = 0)
@

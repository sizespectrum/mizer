...
If you reading this for the first time, it is likely that you have no idea what a \class{MizerParams} object actually is. In 'R-speak' it is an \code{S4} object, which means it is made up of 'slots'. Slots are essentially containers that store the object data. These slots can be seen by running:
...
If you plot this predation mortality on log-log scale you can see how the predation mortality declines to almost zero for the largest sizes (Figure~\ref{fig:plot_comm_m2}). There is also a kink in the predation mortality for sizes that are the prey of the largest sizes - an illustration of a mild tropic cascade which we shall discuss next.
...
This means we need to consider the fishing selectivity function. By default, the fishing gear selectivty function is a sigmoid function based on length (see Section~\ref{sec:fishing_gear} and the help page for the function \code{sigmoid\_length()}). The shape of this selectivity pattern is determined by two parameters: $l25$ and $l50$, which are the lengths at 25 and 50\% selectivity respectively. These lengths are converted to weights using the default length-weight conversion parameters of $a$ = 0.01 and $b$ = 3.0 (the values of these parameters can be changed by passing arguments to the \code{set\_community\_model()} function).
In this example we would like to simulate a knife-edge type of fishing selectivity, with only species larger than 1 kg being selected. This means we have to calculate values for $l25$ and $l50$:
...
We add these parameters to the wrapper function to get our \class{MizerParams} with knife-edge selectivity:

<<label='set_comm_fishing'>>=
params_knife <- set_community_model(z0 = 0.1, recruitment = 4e7, alpha = 0.2, f0 = 0.7, l50 = l50, l25 = l50*0.99)
@
...
You can compare the difference between these scenarios by using the \code{plot()} method as before (Figure~\ref{fig:plot_comm_fmort}). Of particular interest is the fishing mortality at size.
%This can be plotted using the \code{plotFMort()} method .
The knife-edge selectivity at 1000g can be clearly seen.
...
As mentioned above, the trait-based size spectrum model is a simplification of the general size spectrum model (see \citet{andersen_damped_2010} and \citet{andersen_direct_2010}). Unlike the community model described above, more than one species is resolved. However, all the species-specific parameters are the same, except for the asymptotic size, which is considered the most important ``trait'' characterizing a species. Other parameters are related to the asymptotic size, i.e.~$w_m = \eta_m W$, where $\eta_m = 0.25$, i.e. the life history parameters of each species in the model is determined by the most important trait.  The number of species is not important and does not affect the general dynamics of the model.
...
The main parameters of interest are the number of the species in the model (\code{no\_sp}) and the minimum and maximum asymptotic sizes (\code{min\_w\_inf} and \code{max\_w\_inf} respectively). Here we set up the model to have 10 species, with asymptotic sizes ranging from 10 to 1e05 g (the sizes are spread evenly on a logarithmic scale).

<<label=demo_trait_model_params>>=
params <- set_trait_model(no_sp = 10, min_w_inf = 10, max_w_inf = 1e5)
@

This function returns an object of type \class{MizerParams}.

<<label=class_trait_params>>=
class(params)
@

This object can therefore be interrogated in the same way as described in the COMMUNITY SECTION above, either by inspecting the individual slots or by using the \code{summary()} function.

SUMMARY

As with the community model described above, we can project the model through time using the \code{project()} method.
Here we project the model for 100 time steps, without any fishing, and using the default initial population abundances given by the \code{get\_initial\_n()} function (see Section \ref{sec:setting_initial_abundances}).

ARE INITIAL ABUNDANCES MENTIONED IN COMMUNITY SECTION ?

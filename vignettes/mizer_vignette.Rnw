...
\title{Multispecies size spectrum ecological modelling in \R (\pkg{mizer}): An introductory vignette}
%\author{F. Scott, J. Blanchard, K. H. Andersen}
\author[1,4]{F. Scott}
\author[2]{J. Blanchard}
\author[3]{K. H. Andersen}
...
As you probably know, to access documention in \R you can use the \code{help()} function. This can be used to access package documentation in a range of ways. For example:
...
\pkg{mizer} is able implement all three types of model using similar commands.
...
\begin{equation}
  \label{eq:mup}
  \mu_{p.i}(w_p) = \sum_j \int \phi(w_p/w) (1-f_i(w)) \gamma_j w^q \theta_{ji} N_j(w) \, dw.
\end{equation}
...
\end{equation}
where $\overline{\epsilon}_I$ is the average growth efficiency which can be derived from the full model \citep[see][or]{andersen_trophic_2009, zhang_trait_2013}.

In this section we descibe how a community model can be set up and projected through time. We then use a community model to illustrate the idea of a 'tophic cascade'. Due to the relative simplicity of this type of model they are useful for gently introducing some of the concepts behind the \pkg{mizer} package. Consequently, this section should hopefully serve as an introduction to using \pkg{mizer} and some of the main classes and methods.
...
The arguments for \code{project()} that we need to be concerned with are \args{effort}, which determines the fishing effort (and therefore fishing mortality) through time, and $t\_max$, which is the length of the simulation.
...
\begin{figure}
<<label=print_plot_comm_sim, fig=TRUE, echo=FALSE, height = 9>>=
<<plot_comm_sim>>
@
...
\begin{figure}
<<label=print_plot_comm_m2, fig=TRUE, echo=FALSE, height = 4>>=
...
The \code{set\_community\_model()} automatically sets the fishing selectivity to be have knife-edge shape, with only individuals larger than 1 kg selected (the size at the knife-edge can be changed by setting the \args{knife\_edge\_size} argument). Although it is possible to change the selectivity function, here we will use the default knife-edge selectivity. Here we set up the parameter object exactly as before, but now we are explicitly setting the size at which individuals are selected by the fishing gear.
...
\begin{figure}
<<label=print_plot_comm_fmort, fig=TRUE, echo=FALSE, height = 9>>=
<<plot_comm_fmort>>
@
...
\begin{figure}
<<label=print_plot_relative_comm_abund, fig=TRUE, echo=FALSE, height = 4>>=
...
\begin{figure}
<<plot_comm_biomass_sigma1, fig=TRUE, height = 4>>=
...
It is more complicated than a community model and the most significent different between the two is that while the community model only resolves a single ``species'', the trait-based model resolves many species.
...
\begin{figure}
<<label=print_plot_trait_sim, fig=TRUE, echo=FALSE, height = 9>>=
<<plot_comm_sim>>
@
...
\begin{figure}
<<label=print_plot_trait_fmort, fig=TRUE, echo=FALSE, height = 9>>=
<<plot_trait_fmort>>
@
...
Which can be plotted using (Figure~\ref{fig:plot_relative_comm_abund}):
...
plot(x = sim0@params@w, y = m2_no_fishing, log="x", type="n", xlab = "Size (g)", ylab = "M2")
lines(x = sim0@params@w, y = m2_no_fishing, lty=2)
lines(x = sim0@params@w, y = m2_with_fishing)
@

\begin{figure}
<<label=print_plot_relative_trait_m2, fig=TRUE, echo = FALSE, height = 4>>=
...
In the previous simulations we had only one fishing gear and it targetted all the species in the community. This gear had a knife-edge selectivity that only selected species larger than 1 kg. Here we expand the model to include multiple fishing gears.
...
\begin{figure}
<<label=print_plot_trait_multi_gear_single_effort, fig=TRUE, echo=FALSE, height = 9>>=
...
sim_multi_gear <- project(params_multi_gear, t_max = 75, effort = c(Industrial = 0.75, Other = 0))
@
...
\begin{figure}
<<label=print_plot_trait_multi_gear, fig=TRUE, echo=FALSE, height = 9>>=
...
sim_industrial1 <- project(params_multi_gear, t_max = 75, effort = c(Industrial = 0.75, Other = 0))
...
For example, each species will have different life-history parameters; there may be multiple gear types with different selectivities targetting different groups of species; the fishing effort of each gear may change with time instead of just being constant (which has been the case in the simulations we have looked at so far); the interactions between the species needs to be considered.
...
Although many of the arguments used when creaing a \class{MizerParams} object are optional, there is one argument that must be supplied by the user: the \emph{species specific parameters}.  These are stored in a single \class{data.frame} object. The \class{data.frame} is arranged species by parameter, so each column is a parameter and each row has the parameters for one of the species in the model.
...
\begin{table}[ht]
	...
	%\begin{tabularx}{r|X|X}
        ...
        h & Maximum food intake rate. If this is not provided, it is calculated using the \args{k\_vb} column. Therefore, either \args{h} or \args{k\_vb} must be [rovided. & Optional (no default) \\
        k\_vb & The von Bertalanffy $K$ paramter. Only used to calculate \args{h} if that column is not provided & Optional (no default)\\
...
The data can be read into \R using the commmand \code{read.csv()}. This reads in the data as a data.frame. We then need
...
If an interaction matrix is not specified to the \code{MizerParams()} contructor the default interation matrix is used. This has all values set to 1. 
...
As mentioned in the preceeding sections, an object of \class{MizerParams} is created by using the \code{MizerParams()}
...
This means the interaction matrix will be set to the default value (see Section~\ref{sec:interaction_matrix}). This is a matrix of 1s, implying that all species fully interact with each other, i.e. the species are spread homogenously across the model area.
...
forward through time, starting from an initital population abundance and with a pre-determined fishing effort pattern.
...
Running \code{project()} returns an object of type \class{MizerSim} that stores the results of the proejection.
...
<<label = remaking_simple_ms_params>>=
params <- MizerParams(params_data, interaction = inter)
@

As described above, effort is associated with fishing gears.
Because we haven't specfied any gears in the \args{params\_data} species parameter data.frame, each species is caught by a separate gear, named after the species.
...
\begin{figure}
<<label=print_plot_basic_ms_sim, fig=TRUE, echo=FALSE, height = 9>>=
<<plot_basic_ms_sim>>
@
...
As mentioned above, we can also set the effort values for each gear separatley using a vector of effort values. This still keeps the efforts constant through time but it means that each gear can have a different constant effort.
...
\begin{figure}
<<label=print_plotFMort, fig=TRUE, echo=FALSE, height = 4>>=
<<plotFMort>>
@
...
\begin{table}[ht]
	...
	%getPredRate() & &\\
	getM2() & Three dimensional array (time x species x size) & The predation mortality imposed on each species by size throught time.\\
...
\R is very powerful when it comes to exploring data through plots.  A useful package for plotting is \pkg{ggplot2}.  \pkg{ggplot2} uses data.frames for input data. Many of the summary methods and slots of the \pkg{mizer} classes are arrays or matrices. Fortunately it is straightforward to turn arrays and matrices into data.frames using the command \code{melt} which is in the \pkg{reshape} package.
Although \pkg{mizer} does include some dedicated plots, it is definately worth your time getting to grips with these \pkg{ggplot2} and other plotting packages. This will make it possible for you to make your own plots.
...
\begin{figure}
<<label=print_plotBiomass, fig=TRUE, echo=FALSE, height = 4>>=
<<plotBiomass>>
@
...
\begin{figure}
<<label=print_plotSpectra_example, fig=TRUE, echo=FALSE, height = 4>>=
<<plotSpectra_example>>
@
...
\begin{figure}
<<label=plot_example, fig=TRUE, echo=FALSE, height = 9>>=
<<demo_summary_plot>>
@
...
We will set up the model, project it through time using historical levels of fishing effort, and then examine the results.
This example is based on the multispecies model in \citet{blanchard_multispecies_2013}.

%We use data from the North Sea from 1957 to 2010 and compare the simulated population trajectories to the best estimates of abundance from ICES where available.

\subsection{Setting up the North Sea model}

The first job is to set up the \class{MizerParams} object for the North Sea model.
In the previous examples we have already been using the life-history parameters and the interaction matrix for the North Sea model used in \citet{blanchard_multispecies_2013}.
We will use them again here. The parameters and the interaction matrix are stored as *.csv files that need to be read in. We can use the \code{system.file()} function to tell us the location of the files.
Remember that we need to convert the interaction file into a matrix.
...
We have seen before that the species in the model are \Sexpr{paste(params_data[,"species"], collapse=", ")} and
% No idea why the following line doesn't work
that the \args{params\_data} object is a data.frame with columns for \args{species}, \args{w\_inf}, \args{w\_mat}, \args{beta}, \args{sigma}, \args{r\_max} and \args{k\_vb}.
...
We have also seen before that only having these columns is sufficient to make a \class{MizerParams} object. Any missing columns will be supplied by the default values.
...
We will use the default stock-recruitment relationship, which is the Beverton-Holt shape. As we have seen earlire, this requires a value of \args{r\_max} to be set for each species. This column is already in the \args{params\_data} data.frame. The values for \args{r\_max} are taken from \citet{blanchard_multispecies_2013}.
...
Here, we want to take a look at the selectivity of the species. By default, the selectivity function is a knife-edge which only takes a single argument, \args{knife\_edge\_size}.
In this model we want the selectivity pattern to be a sigmoid shape. This can be written in terms of \args{l25} and \args{l50}, which are the lengths at which 25\% and 50\% of the stock is selected. The length based sigmoid selectivity looks like:
...
This selectivity function is implemented in \pkg{mizer} as \code{sigmoid\_length()}.
As well as the arguments \args{l25} and \args{l50}, this function also has arguments \args{a} and \args{b} to convert between length and weight: $w = a l^b$
This is becauser the sigmoid selectivity function is defined in terms of length, and the size spectrum model works in terms of weight.
...
Therefore, columns for \args{l25}, \args{l50}, \args{a} and \args{b} need to be added to the species parameter data.frame.
We also add a column specifying the name of the selectivity function we wish to use.
The data is taken from \citet{blanchard_multispecies_2013}. Note it would probably easier to add this data directly to the *.csv file and then read it in rather than type it in by hand like we do here:

<<label=adding_sel_params>>=
params_data$sel_func <- "sigmoid_length"
params_data$l25 <- c(7.6, 9.8, 8.7, 10.1, 11.5, 19.8, 16.4, 19.8, 11.5, 19.1, 13.2, 35.3)
params_data$l50 <- c(8.1, 11.8, 12.2, 20.8, 17.0, 29.0, 25.8, 29.0, 17.0, 24.3, 22.9, 43.6)
params_data$a <- c(0.007, 0.001, 0.009, 0.002, 0.010, 0.006, 0.008, 0.004, 0.007, 0.005, 0.005, 0.007)
params_data$b <- c(3.014, 3.320, 2.941, 3.429, 2.986, 3.080, 3.019, 3.198, 3.101, 3.160, 3.173, 3.075)
@

Note that we don't set up any gear names here, so each species will be caught by separate gear named after the species.
...
The historical fishing mortality from 1967 to 2010 for each species is stored in the csv file \code{NS\_f\_history.csv}.
As before, we can use the \code{read.csv()} to read in the data. This reads the data in as a \class{data.frame}.
We want this to be a \class{matrix} so we use the \code{as.matrix()} function:
...
We have seen that the fishing mortality in the \code{project()} function is driven by the fishing effort.
...
One aspect of the model parameters that we have not considered so far is the catchability. Catchability is a scalar parameter used to calculate the fishing mortality at size given the selectivity and effort of the gear.
...
We could therefore leave catchability at 1 for all species and then use the \code{f\_history} matrix as the fishing effort.
However, an alternative method which is probably clearer is to use the effort relative to a chosen reference year.
Here we use the year 1990 as the reference year. If we set the catchability of each species to be the same as the fishing mortality in 1990 then a relative effort of 1 in 1990 will result in the fishing mortality being what it was in 1990. The effort in the other years will be relative to the effort 1990.
...
We will use default values for all of the other parameters apart from \args{kappa}, the carrying capacity of the resource spectrum (see Section~\ref{sec:parameters}).
...
Before we can run a projections we need to rescale the effort matrix to get a matrix of efforts relative to 1990. 
To do this we want to rescale the \code{f\_history} object to 1990 so that the relative fishing mortality in 1990 = 1.
...
One way around is this to project forward at a constant fishing mortality equal to the mortality in the first year until equilibrium is reached.
...
Here we make an inital effort matrix of 100 years at the first effort level, complete with dimension names for the time dimension.
We then stick it on top of the original matrix of historical relative effort.
We also need to some dimnames to the time dimension.
...
We use \code{dt} = 0.5 (the simulation will run faster than with the default of 0.1, but tests show that the results are still stable) and save the results every year.
...
You can see the 100 year period of transients with constant fishing fishing mortality as the dynamics approach equilibrium before the historical relative mortality is used.
...
\begin{figure}
<<label=print_plot_ns_biomass, fig=TRUE, echo=FALSE, height = 4>>=
<<plot_ns_biomass>>
@
...
\subsection{Exploring the outputs}

Here we explore the results of the simulation.
%We are going to compare the simulated SSB and yield for each species to the ``official'' values from ICES.
We calculate the community indicators ``mean maximum weight``, ``mean individual weight``, ``community slope`` and the ''large fish indicator''  and compare the results to a community target.
The community target is based on achieving a high proportion of the unexploited value of the large fish indicator (LFI) $0.8 LFI_{F=0}$ \citep{rochet_does_2011}.
...
Here we calculate the LFI and other community indicators for the unexploited community using the \code{getProportionOfLargeFish()} method. The LFI is based on species larger than 40 cm.
As in \citet{blanchard_multispecies_2013}, when calculating these indicators we only include demersal species and individuals in the size range 10 g to 100 kg.

<<label=ns_comm_ref_point>>=
demersal_species <- c("Dab","Whiting","Sole","Gurnard","Plaice","Haddock","Cod","Saithe")
lfi0 <- getProportionOfLargeFish(sim0, species = demersal_species,
    min_w = 10, max_w = 100e3, threshold_l = 40)["100"]
mean_weight0 <- getMeanWeight(sim0, species = demersal_species, min_w = 10, max_w = 100e3)["100"]
mean_max_weight0 <- getMeanMaxWeight(sim0, species = demersal_species, min_w = 10, max_w = 100e3)["100","mmw_biomass"]
slope0 <- getCommunitySlope(sim0, species = demersal_species, min_w = 10, max_w = 100e3)["100","slope"]
@
...
mean_weight <- getMeanWeight(sim, species = demersal_species, min_w = 10, max_w = 100e3)[as.character(years)]
mean_max_weight <- getMeanMaxWeight(sim, species = demersal_species, min_w = 10, max_w = 100e3)[as.character(years),"mmw_biomass"]
slope <- getCommunitySlope(sim, species = demersal_species, min_w = 10, max_w = 100e3)[as.character(years),"slope"]
@

We can plot these up, along with the unexploited levels and the LFI reference level. Here we do it using ggplot by making three data.frames (one for the time series, one for the unexploited levels and one for the reference levels).:
<<label=plot_ns_indicators, eval = FALSE>>=
library(ggplot2)
community_plot_data <- data.frame(year = years, measure = "LFI", data = lfi)
community_plot_data <- rbind(data.frame(year = years, measure = "LFI", data = lfi),
                            data.frame(year = years, measure = "Mean Weight", data = mean_weight),
                            data.frame(year = years, measure = "Mean Max Weight", data = mean_max_weight),
                            data.frame(year = years, measure = "Slope", data = slope))
p <- ggplot(community_plot_data) + geom_line(aes(x=year, y = data)) + facet_wrap(~measure, scales="free")
# Add unfished levels
community_unfished_data <- rbind(data.frame(year = years, measure = "LFI", data = rep(lfi0, length(years))),
                            data.frame(year = years, measure = "Mean Weight", data = rep(mean_weight0, length(years))),
                            data.frame(year = years, measure = "Mean Max Weight", data = rep(mean_max_weight0, length(years))),
                            data.frame(year = years, measure = "Slope", data = rep(slope0, length(years))))
p <- p + geom_line(aes(x=year,y=data), linetype="dashed", data = community_unfished_data)
community_reference_level <- data.frame(year=years, measure = "LFI", data = rep(lfi0 * 0.8, length(years)))
p + geom_line(aes(x=year,y=data), linetype="dotted", data = community_reference_level)
@

\begin{figure}
<<label=print_plot_ns_indicators, fig=TRUE, echo=FALSE, height = 6>>=
<<plot_ns_indicators>>
@
...
As well as investigating the historical simulations, we are also interested in running projections into the future.
...
\begin{itemize}
    ...
    \item From 2010 to 2015, linearly approach $F_{MSY}$ and then continue at $F_{MSY}$ until 2050.
\end{itemize}

We will compare the SSB of each species to a biodiversity target based on the reference point $0.1 SSB_{F=0}$ \citep{rochet_does_2011}.
...
scenario1 <- t(array(relative_effort["2010",], dim=c(12,40), dimnames=list(NULL,year = 2011:2050)))
scenario1 <- rbind(relative_effort, scenario1)
@

The relative effort array for the second scenario is more complicated to make and requires a little bit of \R gymnastics (it might be easier for you to prepare this in a spreadsheet and read it in).
For this one we need values of $F_{MSY}$.  
<<label = ns_scenario2_relative_effort>>=
fmsy <- c(Sprat = 0.2, Sandeel = 0.2, N.pout = 0.2, Herring = 0.25, Dab = 0.2, Whiting = 0.2,
    Sole = 0.22, Gurnard = 0.2, Plaice = 0.25, Haddock = 0.3, Cod = 0.19, Saithe = 0.3)
...
Both of our new effort scenarios still include 100 years at the 1967 level to deal with transisent behaviour.
...
We can now compare the projected SSB value to the biodiversity reference points.
First we calculate the biodiversity reference points (from the final time step in the \args{sim0} simulation):
...
We make use of the \code{melt()} function in the very useful \pkg{reshape} package (REF).
...
ssb1_df <- melt(ssb1)
ssb2_df <- melt(ssb2)
ssb_df <- rbind(cbind(ssb1_df, scenario = "Scenario 1"),
                cbind(ssb2_df, scenario = "Scenario 2"))
ssb_unexploited_df <- cbind(expand.grid(sp = names(ssb0), time = 1967:2050),value = as.numeric(ssb0), scenario = "Unexploited")
ssb_reference_df <- cbind(expand.grid(sp = names(ssb0), time = 1967:2050),value = as.numeric(ssb0*0.1), scenario = "Reference")
ssb_all_df <- rbind(ssb_df, ssb_unexploited_df, ssb_reference_df)
p <- ggplot(ssb_all_df) + geom_line(aes(x = time, y = value, colour = scenario)) + facet_wrap(~sp, scales = "free", nrow = 4)
p
@

\begin{figure}
<<label=print_projected_ssb_ns, fig=TRUE, echo=FALSE, height = 9>>=
<<projected_ssb_ns>>
@
\caption{Historical and projected SSB under two fishing scenarios}
\label{fig:plot_ns_comm_indicators}
\end{figure}

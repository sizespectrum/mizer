...
Objects of class \class{MizerParams} are created using the constructor method \code{set\_multispecies\_model()}.
...
In the rest of this section we look at the main arguments to the \code{set\_multispecies\_model()} constructor method.
...
This mean that if an \args{h} column is not included in the species parameter data.frame, a column for \args{k\_vb} is necessary. If it is not included then the \code{set\_multispecies\_model()} function will fail with an error message. The calculation also requires a value of the feeding level of small individuals. This can be passed as an additional argument, \args{f0}, to the \code{set\_multispecies\_model()} constructor and it has a default value of 0.6.
...
If an interaction matrix is not specified to the \code{set\_multispecies\_model()} constructor the default iteration matrix is used. This has all values set to 1. 

\subsection{The other arguments}
...
\begin{table}[ht]
    ...
    z0exp & See z0pre & n - 1\\
	\end{tabularx}
	\caption{Other parameters to the \code{set\_multispecies\_model()} constructor}
...
As mentioned in the preceding sections, an object of \class{MizerParams} is created by using the \code{set\_multispecies\_model()}
...
This species parameter data.frame is the minimum we need to create a \class{MizerParams} object as it contains only essential columns. To create the \class{MizerParams} object we pass the data.frame into the \code{set\_multispecies\_model()} constructor method:

<<label=first_MP, results='hide'>>=
params <- set_multispecies_model(params_data)
@
...
params200 <- set_multispecies_model(params_data, no_w=200)
summary(params200)
@
...
We will use default values for the remainder of the arguments to the \code{set\_multispecies\_model()} function (see Section~\ref{sec:other_mizerparams_arguments} and Table~\ref{tab:other_params}).
...
\begin{table}[ht]
	...
	plot() & Produces 5 plots (\code{plotFeedingLevel()}, \code{plotBiomass()}, \code{plotPredMort()}, \code{plotFMort()} and \code{plotSpectra()}) in the same window as a summary.\\
	\hline
	\end{tabularx}
...
params_data$gear <- params_data$species
@
...
% displayFrames(b_trait, b, params)
% @
% 
% 
% 
...
\subsection{Resources in the scale invariant model}

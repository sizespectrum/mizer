...
The main parameters of interest are the number of the species in the model (\args{no\_sp}), the minimum egg size (\args{min_egg}), the minimum maturity size of a species (\args{min_w_mat}) and the minimum and maximum asymptotic sizes (\args{min\_w\_inf}) and (\args{max\_w\_inf}) respectively. The egg, maturity and asymptotic sizes are spread evenly on a logarithmic scale. The stock recruitment relationship is controlled by the value of (\args{r_fac}), which is the ratio of the maximum recruitment $R_{max.i}$ and the actual recruitment $R_i$ at our steady state. Setting \args{r_fac=Inf} corresponds to setting $R_{max.i} \rightarrow \inf,$ which corresponds to turning the stock recruitment relationship off.  

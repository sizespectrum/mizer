...
In the previous simulations we have had only one fishing gear and it targetted all the species in the community. Here we expand the model to include multiple fishing gears.
...
This is useful because when running a simulation with \code{project()} you can specify the effort per gear.
 
Here we implement an industrial type fishery. The industrial fishery will only target species that have an asymptotic size less than or equal to 500 g.
We will still use a knife-edge selectivity but here we want the position of the knife-edge to depend on the asymptotic size of the species it fishes.
The position of the knife-edge will occur at 0.05 x the asymptotic size i.e. the selectivity pattern will be different for each species and will depend on the asymptotic size.
...
This might sound complicated but is straight forward to do.

In the previous examples, we passed an argument \args{knife\_edge\_size} to the \code{set\_trait\_model()} function.
\args{knife\_edge\_size} was passed in as a single value which was used for all the species. This effectively set up a single gear, that caught all species using the same selectivity pattern.
...
This will set up a single gear for each species in the model.

To start with we need to know what the asymptotic sizes of the species in the model are. As mentioned above, the \code{set\_trait\_model()} function spaces the asymptotic sizes equally on a logarithmic scale. This means we can calculate them by hand.
...
Finally, we can create our \class{MizerParams} object by passing in the $knife\_edge\_sizes$ argument. ALl the other arguments are the same as before:
...
The columns of the \code{species\_params} \class{data.frame} that we are interested in here are \args{species} (the name of the species, here just numerical identifiers), \args{w\_inf} (the asymptotic size), \args{sel\_func} (the name of the selectivity function for that species), \args{knife\_edge\_size} (the position of the knife-edge of the selectivity) and \args{gear} (the name of the fishing gear, note that here each name is unique signifying that each species is caught by a different gear).
...
When specifying the \args{effort} argument for \code{project()} there are several ways of doing it. Above we specified a single value that was used for all gears, for all time steps. It is also possible to specify a separate effort for each gear. This means that we can ``turn off'' the effort for the gears that are fishing on species with an asymptotic size greater than 500 g by setting the effort to 0.
To do this we can create a vector of the gears that we want to be operational (those that fish species with an asymptotic size less than or equal to 500 g) and then multiply this by the desired effort value:
...
By passing this vector into \code{project()} as the \args{effort} argument we can effectively switch on and off the gears we want:
...
\begin{table}[ht]
	...
	%\begin{tabularx}{r|X|X}
        ...
        h & Maximum food intake rate. If this is not provided, it is calculated using the \code{k\_vb} column. Therefore, either \code{h} or \code{k\_vb} must be [rovided. & Optional (no default) \\
        k\_vb & The von Bertalanffy $K$ paramter. Used to calculate \args{h} if that column is not provided & Optional (no default)\\
	    gamma & Volumetric search rate. If this is not provided, it is calculated using the \code{h} column and other parameters. & Optional (no default)\\
        ...
        z0 & Background mortality (constant for all sizes). If this is not provided then \args{z0} is calculated as \args{z0pre} * \args{w\_inf} \^ \args{z0exp}. \args{z0pre} and \args{z0exp} have default values of 0.6 and -1/3 respectively.  & Optional (no default)\\

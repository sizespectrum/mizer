...
models the stock is more sensitive to changes in the enviroment, because it 
...
spectrum designed to restrict the trait-based model Section~\ref{sec:trait_based_model} to a scale-invariant form see below, for which the steady-state is known
analytically {\bf(Cuesta et al; well strictly I suppose it's just the 
continuous version for which that's true)}.  Second, in Section~\ref{sec:explicit}, 
these species are replaced, one at a time, by foreground species 
with specified properties in a fish assemblage.  

We shall describe how to run our scale invarient model, and make some comparisons with the old trait based model. Details about our scale invariant model and its motivations are given in {\bf the appendix}.
...
One can see from the plot of time versus biomass that the system is initiated very close to a steady state, and it is further attracted to the steady state without the need to impose any stock recruitement relationship.

\subsection{Comparing with the trait based model}


By comparing Figure~\ref{fig:plot_scale_demo} with Figure~\ref{fig:plot_trait_demo} one can see that the output of the scale-invariant trait-based model is somewhat similar to that of the previous trait-based model. The key difference between these two models is that a strong stock recruitment relationship is present in the original trait based model. In the original trait based model the number of fish grow rapidly, initially, until the population growth is eventually halted by the stock recruitment relationship. If one removed the stock recuitment relationship, the populations in the original trait based model would continue to grow rapidly, but they are being \emph{held back} by the stock recruitment relationship. The result is that the stock in the original trait based model is somewhat insensitive to changes in the enviroment, because its steady state is the result of a strong force for population growth being halted by a strong breaking force (the stock recruitment relationship) . To see this let us run the original trait based model, with a large amount of new effort put into fishing:
...
In Figure~\ref{fig:plot_trait_ins} the large fishing effort just pushes down the final abundance of effected species, but these still remain because of their strong inclination towards population growth. In Figure~\ref{fig:plot_scale_sense}, on the other hand, the heavily fished species are actually going extinct. In summary the scale-invariant trait based model is more sensitive to the effects of fishing, and other changes in the envioment, unlike the original trait-based model which is somewhat held in place by a strong stock recruitment relationship. 
...
\begin{itemize}
 \item By default the scale-invariant trait-based model is initialized at a steady state that we know that analytic form of (see {\bf appendix ?}), whereas the original 
...
\item In order for our model to be scale invariant we require that the exponent of the resource spectrum, the exponent of metabolic intake and the exponent of the volumetric search rate are related by the equation $\lambda = 2-n+q.$  
...
%We have to choose parameters so that $\mu _0$ is sufficiently large that $ \mu_{b.i}(w) = \mu_0 w^{n-1} - \mu_{f.i}(w) - \mu_{p.i}(w)\geq 0. $ 
 \item The resource spectrum extends from the smallest weight at which an explicitly modeled fish may eat, up to the minimum maturity weight of a fish. We define the carrying capacity $c_p(w)$ of the background resource so when our system is at steady state, the 
combination of the background resource spectrum and the aggregate abundance of the explicity modeled species is the power law $\kappa w^{-\lambda}.$ The details of how $c_p(w)$ is set within the scale-invariant model are given in the appendix. 
...
\item In order for us to be able to obtain a closed form expression for a steady state, we assume that the proportion of energy allocated to reproduction $\psi_i(w)$ is given by the step-like function
\begin{equation}
\psi_i(w)=\begin{cases}
...
\end{cases}
\end{equation}
This can be achieved by taking the form given by  (Equation~\ref{eq:psi}), and letting $u \rightarrow \infty.$ Note that this is not a requirement for scale invariance, and the only reason this assumption is made is that it is mathematically 
convenient is that it allows to obtain an exact analytic expression for our steady state.
\end{itemize}
...
\begin{itemize}
 \item {\bf What is scale invariance?}  
  ...
  Consider a second species with characteristic weight $cW$ that is multiplied by a 
  positive rescaling factor $c$.  Then (Equation~\ref{eq:MvF}) would be 
  scale-invariant, if the latter species had dynamics identical to the first 
  when rescaled as $c^{-\lambda - 1} N(c w, c^{1-n} t),$ where $\lambda$ is the exponent of the resource spectrum, and $n$ is the exponent of for maximum food intake. This rather
  ...
  to the scale-invariant system. We avoid the problem of having to model 
  arbitrarily small species by replacing them with the background resource, so the 
 ...
 \item {\bf Why use scale invariance?} 
...
\subsection{Setting up a scale-invariant assemblage at steady state}


The modifications to the trait-based model are implemented in the function
{\bf set\_scaling\_model}.

{\bf I think some words equivalent to those in Section 6.2 are going to 
be needed here.  Obviously this requires detailed knowledge of the code in
set\_scaling\_model, and one of you will need to write this.}

\subsection{Scale-invariant assemblage: example of dynamics and steady state}

.{\bf This needs an example to be run.} 

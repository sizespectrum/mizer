...
You can compare the difference between these scenarios by using the \code{plot()} method as before. Of particular interest is the fishing mortality at size. This can be plotted using the \code{plotFMort()} method (Figure~\ref{fig:plot_comm_fmort}). The knife-edge selectivity at 1000g can be clearly seen.
...
<<plot_comm_fmort>>
@
\caption{Fishing mortality at size for the community model with knife-edge selectivity at size = 1000 g.}

...
There are several advantages to this approach. As well investigating the community as whole (as was done for the community and trait-based models), we are able to investigate the dynamics of individual species. For example, species specific management rules can be tested and species specific metrics, such as MSY, can be compared to reference levels.

In a food-web model each species will have different life-history parameters. This means that this type of model can be harder to set up
Additionally, there may be multiple gear types with different selectivities targetting different groups of species (and some species may be targeted by more than one gear); the fishing effort of each gear may change with time instead of just being constant (which has been the case in the simulations we have looked at so far); the interactions between the species needs to be considered.
...
This will all be done in the context of examples so hoperfully everything will be clear.
...
You can also the help page for the class by entering \code{class ? MizerParams}

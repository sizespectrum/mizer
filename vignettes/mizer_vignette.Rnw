...
% eq:psi
% eq:mu_b 
\section{Implementing a scale-invariant trait-based model}
...
enough within species to guarantee persistence of the species.  They do this 
...
rather small values of $R_{max.i}$.

This and Section~\ref{sec:explicit} provide an alternative way of generating
sets of coexisting species without strong stock-recruitment relationships. In these 
models the stock is more sensitive to changes in the environment, because it 
is allowed to react to such changes naturally, rather than being heavily controlled 
by a stock recruitment relationship. The key is a special power-law between body weight and density, which marine ecosystems are often observed to be near {\bf(Sheldon)}.  
On the exact power-law, it can be proved that an arbitrary number of 
species coexist {\bf(Cuesta et al)}. This is not to suggest 
that real assemblages of fish species are at rest on the exact power-law. 
Rather it is to construct a \emph{background} assemblage of a form that 
facilitates coexistence of observed fish species (\emph{foreground} species), without calling for strong stock-recruitment relationships. 

We do this in two steps.  First, this section describes a background
spectrum designed to restrict the trait-based model (see Section~\ref{sec:trait_based_model}) to a scale-invariant form (see Section~\ref{sec:appendix}), for which the steady-state is known
analytically (Equation~\ref{eq:steady}).  Second, in Section~\ref{sec:explicit}, 
these species are replaced, one at a time, by foreground species 
with specified properties in a fish assemblage.  

We shall describe how to run our scale invariant model, and make some comparisons with the old trait based model. Details about our scale invariant model and its motivations are given in Section~\ref{sec:appendix}.

\subsection{Implementing the scale invariant model}


To help set up a scale invariant trait-based model, there is a wrapper function, \code{set\_scaling\_model()}. Like the \code{set\_community\_model()} and \code{set\_tait\_model()} functions described above, this function can take many arguments. Most of them have default values so you don't need to worry about them for the moment. See the help page for more details.

<<label=help_set_scaling_model, eval=FALSE>>=
?set_scaling_model
@

The main parameters of interest are the number of the species in the model (\args{no\_sp}), the minimum egg size (\args{min\_egg}), the minimum maturity size of a species (\args{min\_w\_mat}) and the minimum and maximum asymptotic sizes (\args{min\_w\_inf}) and (\args{max\_w\_inf}) respectively. The egg, maturity and asymptotic sizes are spread evenly on a logarithmic scale. The Beverton-Holt stock recruitment relationship is turned off by default.  
...
Once a \class{MizerParams} object has been created for the scaling invariant model it can be run in the usual way:

<<label = scale_run>>=
sim <- project(params, t_max=5, effort = 0)
@

Let us make a picture of the results:
...
\begin{figure}
<<label=print_plot_scale_sim, echo=FALSE, fig.height=9>>=
<<plot_scale_sim>>
@
\caption{Example plot of the scale invariant trait-based model with no fishing.}
\label{fig:plot_scale_demo}
\end{figure}

One can see from the plot of time versus biomass that the system is initiated very close to a steady state, and it is further attracted to the steady state without the need to impose any stock recruitment relationship (at least under default settings).

\subsection{Comparing with the trait based model}


By comparing Figure~\ref{fig:plot_scale_demo} with Figure~\ref{fig:plot_trait_demo} one can see that the output of the scale-invariant trait-based model is somewhat similar to that of the previous trait-based model. The key difference between these two models is that a strong stock recruitment relationship is present in the original trait based model. In the original trait based model the number of fish grow rapidly, initially, until the population growth is eventually halted by the stock recruitment relationship. If one removed the stock recruitment relationship, the populations in the original trait based model would continue to grow rapidly, but they are being \emph{held back} by the stock recruitment relationship. The result is that the stock in the original trait based model is somewhat insensitive to changes in the environment, because its steady state is the result of a strong force for population growth being halted by a strong breaking force (the stock recruitment relationship) . To see this let us run the original trait based model, with a large amount of new effort put into fishing:


<<label=demo_trait_model_insensitive>>=
params_trait <- set_trait_model(no_sp = 10, min_w_inf = 10, max_w_inf = 1e5,
                          knife_edge_size = 100)
sim_trait <- project(params_trait, t_max=5, effort = 100)
@

We make a plot of how biomass changes over time (Figure~\ref{fig:plot_trait_ins}).  

<<plot_trait_ins, eval=FALSE>>=
plotBiomass(sim_trait)
@

\begin{figure}
<<label=print_plot_trait_ins, echo=FALSE, fig.height=4, fig.keep='last'>>=
<<plot_trait_ins>>
@
\caption{A biomass plot of the original trait-based model under significant fishing.}
\label{fig:plot_trait_ins}
\end{figure}


Notice that even though the fishing mortality rate is very high (orders of magnitude more than the predation mortality rate), no species are going extinct. Let us compare this with the analogous situation, where a high fishing effort is put into the scale-invariant trait-based model. We run the system under a high fishing effort:

<<label=demo_scale_model_sensitive>>=
params <- set_scaling_model(no_sp = 10, min_w_inf = 10, max_w_inf = 1e3,
                            min_egg = 1e-4, min_w_mat = 10^(0.4), rfac = Inf,
                            knife_edge_size = 100,kappa = 0.005)
sim <- project(params, t_max=5, effort = 100)
@

We make a plot of how biomass changes over time (Figure~\ref{fig:plot_scale_sense}).   

<<plot_scale_sense, eval=FALSE>>=
plotBiomass(sim)
@

\begin{figure}
<<label=print_plot_scale_sense, echo=FALSE, fig.height=4, fig.keep='last'>>=
<<plot_scale_sense>>
@
\caption{A biomass plot of the scale-invariant trait-based model under significant fishing.}
\label{fig:plot_scale_sense}
\end{figure}

In Figure~\ref{fig:plot_trait_ins} the large fishing effort just pushes down the final abundance of effected species, but these still remain because of their strong inclination towards population growth. In Figure~\ref{fig:plot_scale_sense}, on the other hand, the heavily fished species are actually going extinct, which seems more realistic. In summary the scale-invariant trait based model is more sensitive to the effects of fishing, and other changes in the environment, unlike the original trait-based model which is somewhat held in place by a strong stock recruitment relationship. 

\subsection{Modifications to the trait-based model}

The scale invariant trait-based model corresponds to a special case of the general size spectrum model described in Section~\ref{sec:principles}.  Essentially the scale invariant trait-based model is very similar to the original trait-based model of Section~\ref{sec:explicit}, however there are the following differences:

{\bf Richard.L: could you rephrase these as you see fit ? }
\begin{itemize}
 \item By default the scale-invariant trait-based model is initialized at a steady state (Equation~\ref{eq:steady}) (provided no fishing is being performed), whereas the original 
 trait based model is not initialized near its steady state.
 \item In order for our model to be scale-invariant we require that the exponent of metabolic intake $n$ equals the 
exponent $p$ of metabolic loss. 
\item In order for our model to be scale invariant we require that the exponent of the resource spectrum $\lambda$, the exponent of metabolic intake $n$ and the exponent of the volumetric search rate $q$ are related by the equation $\lambda = 2-n+q.$  
\item Scale invariance requires similarity on every size scale, and so we cannot have any special sizes such as a constant egg weight shared by all species. In fact, we 
require that the egg size $w_{0,i}$ of each species is equal to a fixed fraction of their asymptotic size. Although there is some evidence of such variations in egg size, the common belief is 
that the egg size of most fish is constant at around one milligram, and so our assumption of egg size being proportional to asymptotic size could be taken as an idealization for mathematical simplicity. 
 \item Although a stock recruitment relationship is not required to create a steady state in the scale-invariant trait-based model (unlike the original trait-based model), a stock recruitment relationship can be included in the in the scale-invariant trait-based model by setting the value of \args{r\_fac}. Here \args{r\_fac} equals the maximum recruitment $R_{max.i}$ divided by the actual recruitment $R_i$ at our steady state. By default the stock recruitment relationship is turned off, by setting \args{r\_fac} to go to infinity. This has the effect of also making $R_{max.i}\rightarrow \infty$, and so Equation~\ref{eq:R} is effectively replaced with the simple relationship $R_i = R_{p.i}.$ 
\item The total mortality
 ...
 mortality.  Thus, rather then defining background mortality by Equation~\ref{eq:mu_b} we have a somewhat complicated expression for the background mortality $\mu_b(w)$ given by Equation~\ref{eq:newmub}. Essentially this term represents the predation mortality which would be induced by predators beyond the size ranges we explicitly model at -under the assumption that the abundance of such predators continue the scale invariant pattern. 
...
%We have to choose parameters so that $\mu _0$ is sufficiently large that $ \mu_{b.i}(w) = \mu_0 w^{n-1} - \mu_{f.i}(w) - \mu_{p.i}(w)\geq 0. $ 
 \item The resource spectrum extends from the smallest weight at which an explicitly modeled fish may eat. We define the carrying capacity $c_p(w)$ of the background resource so when our system is at steady state, the 
combination of the background resource spectrum and the aggregate abundance of the explicitly modeled species is the power law $\kappa w^{-\lambda}.$ An expression for how we set $c_p(w)$ within the scale-invariant trait-based model is given by Equation~\ref{eq:cp}. 
...
\item {\bf this piece wants dropping if we dont use step-like psi} In order for us to be able to obtain a closed form expression for a steady state, we assume that the proportion of energy allocated to reproduction $\psi_i(w)$ is given by the step-like function
\begin{equation}
\psi_i(w)=\begin{cases}
...
\end{cases}
\end{equation}
This can be achieved by taking the form given by Equation~\ref{eq:psi}, and letting $u \rightarrow \infty.$ Note that this is not a requirement for scale invariance, and the only reason this assumption is made is that is so we can get a closed form steady state expression. 
\end{itemize}

Note that scale invariance in predation exists as standard feature in the
mizer software tool (Equation~\ref{eq:MvF}), because predation depends on the ratio of predator
to prey weight.  Thus no modification to growth and mortality caused by 
predation is needed when the scale-invariant model is implemented.
...
In Section * we saw that using strong stock recruitment relationships, as implemented within the models discussed in Sections * and * can lead to scenarios where the stock levels are unrealistically insensitive to changes in the envioment. In order to fix this issue we want to create a model where species of interest can co-exist and interact without imposing a strong stock recruitment relationship. The key to our approach for doing this is to use species from the scale-invariant trait based model as a type of background or substrait, into which the heterogeneous species of specific interest (the \emph{foreground species}) can added. The scale-invariant background species essentially act to model the presence of species that one has little knowledge about, or interest in modelling explicitly. 

The key to our approach (of creating a system where species coexist without imposing strong stock recruitment relationships), is that when we add a new species to our system, we do so in such a way that the resulting system will still be at steady state. To do this, we try to ensure that the background species experience the same growth rates and death rates as they did in the previous scale-invariant system. One can make an analogy here with other transplants of biological organs/organisms, where post transplant survivle is based on the transplanted entity and its surrounds recieving similar stimulii before and after the transplant. We ensure that the background species encounter the same growth rates after the transplant as they did before by depressing the abundance of the background species to \emph{make room} for the newly added species, so that the aggregate abundance of all the species remains at the power law. We do not however ensure that the death rates of background individuals will be the same as those in the original system. However, if the newly added species is put in at low abundance, it will cause little change to the death rates of the pre-existing background species, and hence the abundances of the background species will remain stationary as they were before. The abundance curves of the newly added species are calculated under the assumption that their growth and death rates are the ones they would experience in the scale invariant system (as if they were at such a low abundance that self-interaction can be neglected). The more abundant the foreground species is, relative to the background, the worse our approximation to a steady state will be. However in practice, our initial condition often tends to be close enough to a true steady state that it will be attracted to it under the system dynamics.

\subsection{Adding a foreground species to a scale-invariant background}


Initially, when we start with the scale-invariant trait-based model, these species are organized, so that their aggregate abundance follows a power-law. This can be done using the \code{set\_scaling\_model()} command (discussed in Section *) as follows:
...
Let us introduce a species which we are interested in modelling explicityly into this background using our \code{add\_species()} command. For the sake of example we shall introduce a red mullet, we start by specifying its parameters using a dataframe with a single row:
...
<<label=mullet_add>>=
params_out <- add_species(params, species_params, biomass = 2*10^9, 
min_w_observed = species_params$w_mat, rfac = 2)
@

This code creates a new \class{MizerParams} object containing the original background species and the new foreground species, mullet. The \code{add\_species()} command attempts to construct a new approximate steady state (\args{params\_out@initial\_n})
including the new species. This is done by decreasing the abundance of the pre-existing background species, so that when the new foreground species is added (at an abundance such that the total biomass of individuals above weight \args{min\_w\_observed} is \args{biomass}), the aggregate abundance of all species in the system will still given by the power-law $\kappa w^{-\lambda}.$ 

The abundance curve of the newly added species would correspond to a steady state in the case where its abundance (as controlled by the value of \args{biomass}) is neglidible compared to the abundance of the background species. As one increases the value of \args{biomass} this approximation becomes less acceptable, and the initial condition given by \args{params\_out@initial\_n}
will be further from a steady state. Another problem with setting the value of \args{biomass} too high is that if the abundanced of the newly added species exceeds that of the target power-law, it will obviously be impossible that including background species as well will yield the power-law. The code will return an error message about negative abundance multipliers in this case. One way to fix such problems (about the background community being insufficiently abundant to support the value of \args{biomass} of the species one wishes to add) is to increase the value of \args{kappa} in the \code{set\_scaling\_model()} command. 

The stock recruitment relationship can be controlled by setting the value of \args{r\_fac}, in a similar way to as described in Subsection *.
...
We now picture the results (Figure~\ref{fig:plot_mullet_demo}):
...
\begin{figure}
<<label=print_plot_mullet_sim, echo=FALSE, fig.height=9>>=
<<plot_mullet_sim>>
@
\caption{Example plot of the scale invariant trait-based model plus mullet with no fishing.}
\label{fig:plot_mullet_demo}
\end{figure}

\subsection{Adding another species}

In practical fisheries science one will have many species that one wishes to model explicitly. Further species can be added into our system by repeated use of the \code{add\_species()} function. Let us demonstrait this by adding hake into our system with background and mullet. 
...
currency in ecology, and this section can be skipped.  However the approach 
...
for the most part familiar and widely documented in ecology {\bf(Peters book?)}. 


 {\bf What is scale invariance?}  
  {\bf Gustav: can you see if this makes sense. It is hard to define this stuff without going in to all the details}  
  Consider a species with a characteristic weight $W$, and with a density 
  function $N(w,t)$, described by the size-spectrum model, (Equation~\ref{eq:MvF}). 
  Consider a second species with characteristic weight $cW,$ where $c$ is a 
  positive rescaling factor. Then Equation~\ref{eq:MvF} would be 
  scale-invariant, if the latter species had dynamics the same as the first when rescaled as $c^{\lambda + 1} N(c w, c^{1-n} t),$ for any $c>0.$ Here $\lambda$ is the exponent of the resource spectrum, and $n$ is the exponent of for maximum food intake. This rather
  delphic statement basically says that, under scale-invariance, knowing the
  dynamics of one species is enough to know the dynamics of every species.
  Strictly speaking, it could only apply to Equation~\ref{eq:MvF} if there was a 
  continuum of characteristic weights -one species with each characteristic weight $W > 0$. A size-spectrum 
  model based on a finite number of species is, at best, just an approximation 
  to the scale-invariant system. We avoid the problem of having to model arbitrarily small species by replacing them with the background resource, so the 
 ...
 {\bf Why use scale invariance?} 
 Scale-invariant systems possess symmetries which make them easier to understand
 mathematically.  For instance, the assumption of scale invariance in the 
 system with a continuum of characteristic weights allows an exact analytic expression for a steady state of the system to be derived. Although scale invariance cannot be 
 met exactly  in the system with a finite number of species, it can still 
 guide the choice of parameter values that are likely to permit coexistence 
 of species without calling for strong stock-recruitment relationships.

\subsection{A steady state solution}
  {\bf I need to check this makes form is ok, account for psi, and 
  maybe add a description of Gustav's new normalization factor}

...
Roughly speaking there are three versions of the size-based modelling framework of increasing complexity: The \emph{community} model, in which only one ''species'' is resolved; 
...
and the \emph{food-web} in which multiple ''real'' species are resolved, each of which can have differing species-specific traits. 
The community and trait-based models can be considered as simplifications of the food-web type model.
...
This vignette starts by summarising the principles and assumptions of size-based models. The \pkg{mizer} package is then introduced by showing how to set up the simplest types of size-based models: the community and trait-based model. These sections give a basic overview of the classes and methods used by \pkg{mizer}. Simple examples are given that demonstrate how trophic-cascades can be simulated. There then follows a more detailed description of the \pkg{mizer} classes and methods using a food-web model of the North Sea as an example. Finally, there is a detailed example of a size-based model of the North Sea, including a recreation of historical fishing patterns and biomasses.
...
%----------------------------------------------------------------------------------
\section{Size-based modelling - concepts, processes and assumptions}
\label{sec:principles}

Size-based, or size spectrum, models have emerged as a conceptually simple way to model a large community of individuals which grow and change trophic level during life.
...
This section introduces the central assumptions, concepts, processes, equations and parameters of a size-based model.
...
\citep{benoit_continuous_2004, maury_modeling_2007,blanchard_how_2009,law_size-spectra_2009}, the \emph{trait-based} size spectrum model \citep{andersen_asymptotic_2006, andersen_damped_2010}, and the \emph{food-web} spectrum model \citep{hartvig_food_2011-1}.
The community and trait-based models can be considered as simplifications of the food-web type model.
This section focuses on the food-web type of model but is also applicable to the community and trait-based models.
...
With \pkg{mizer} is is possible to implement the three types of model mentioned above (with increasing complexity): community, trait-based and food-web, using the same basic tools and methods.
For the remaineder of this vignette we present examples of how to set up, project and analyse all three types of model.
...
We start by looking at the simplest type of size-based model, the community model. We then move on to a more complex type of model, the trait-based model.
Finally we look at the most complex type of model, the food-web model.
...
The simplest version of the size-based model is the community model, originally introduced by \citep{benoit_continuous_2004}.
...
%---------------------------------------------------
\section{Introducing food-web models}
...
We now turn our attention to food-web, or species-specific, models.
...
In food-web type models multiple species are resolved. However, unlike in the trait-based model which also resolves multiple species, explicit species are represented.
There are several advantages to this approach. As well as investigating the community as a whole (as was done for the community and trait-based models), we are able to investigate the dynamics of individual species. This means that species specific management rules can be tested and species specific metrics, such as MSY, can be compared to reference levels.

A food-web model can be harder to set up.
...
For the remainder of this vignette we build up a food-web model for the North Sea.
To effectively use \pkg{mizer} for a food-web model we are going to have take a closer look at the \class{MizerParams} class and the \code{project()} method.
...
\section{Setting up the model with the class{MizerParams} class}
...
To set up a food-web model we need to directly create and use the \class{MizerParams} class.
...
In size-based modelling recruitment refers to the flux of individuals that enter
the size-spectrum at the smallest size group of that species (given by the
parameter \args{w\_min} in the species parameter data.frame).
As can be seen in Section~\ref{sec:principles}, calculating the recruitment flux involves calculating the ``density independent''
recruitment, $R_{p.i}$ (see Section~\ref{sec:reproduction}).
The $R_{p.i}$ is then modified by a stock-recruitment relationship (SRR) to impose some form of density-dependence. This then results in the
density-dependent recruitment, $R_i$ (see Section~\ref{sec:recruitment}).
...
An example .csv file has been included in the package. This contains the species parameters for a North Sea model (REFERENCE).
...
So far we have created a \class{MizerParams} object using only the species parameter data.frame.
...
We can now run a simple projection with this \class{MizerParams} object. As we have already seen in the community and trait-based model sections, projections are performed using the \code{project()} method. We will look at projections in more detail in Section~\ref{sec:projection}. For now, we will use it in the simplest way, projecting forward for 50 time steps, with no fishing:

<<label=simple_project_food>>=
sim <- project(params, t_max = 50, effort = 0)
@

The basic summary plot can then be called:

<<label=plot_basic_food_sim, eval=FALSE>>=
plot(sim)
@

\begin{figure}
<<label=print_plot_basic_food_sim, fig=TRUE, echo=FALSE, height = 9>>=
<<plot_basic_food_sim>>
@
\caption{Plot of the North Sea food-web model with no fishing.}
\label{fig:plot_food_demo}
\end{figure}

Without fishing the community settles down to equilbria at about t = 50 (Figure~\ref{fig:plot_food_demo}). The big difference between this food-web style model and the trait-based model can be seen in the range of M2 and feeding level values. With the trait based model all the ``species'' had same M2 and feeding level pattern. Here the species all have different patterns, driven by their differing life history characteristics and the heterogeneous interaction matrix.

We investigate what happens when we start fishing in Section XXXX.

%-------------------------------------------------------------------------------------
\subsection{Setting different gears}
\label{sec:setting_different_gears}

MAYBE MOVE BELOW TO PROJECTIONS
...
Here we set up four different gears: Industrial, Pelagic, Beam Trawl and Otter Trawl, that catch different combinations of species.
...
One way of checking the gears in the new \class{MizerParams} object have been set correctly by examining the \args{catchability} slot.
The catchability is a two dimensional array, gear x species (see Equation~\ref{eq:sel}) that acts as an additional multiplier for the fishing mortality imposed on a gear on a species. 
<<>>=
params_gears@catchability
@

You can see that as we wanted we have \Sexpr{dim(params_gear@catchability)[1]} gears that fish on the \Sexpr{dim(params_gear@catchability)[2]} species.
A catchability of 0 means that a species is not caught by that gear.
As we didn't include a \args{catchability} column in the species parameter data.frame the non-zero catchabilities have been set to the default value of 1.
...
In the next section we will see that to project the model through time you need to specify the fishing effort for each gear through time.
...
Another method for setting the fishing effort is to use a numeric vector that has the same length as the number of gears.
The values in the vector are used as the fishing effort of each gear at each time step, i.e. again, the fishing effort is constant through time but each now gear can have a different constant effort.
...
Each row of the array has the effort values of each fishing gear at a time step (the time step is set by the argument \args{dt}, see above).
This means that it is not necessary to supply a \args{t\_max} argument in this case because the maximum time of the simulation is calculated by the number of the time steps in the effort array (the first dimension) and the value of \args{dt}. If a value for \args{t\_max} is also supplied it is ignored.
The columns of the effort matrix must have the same name as the gears in the \code{MizerParams} object.

% Can we think of a way of 'filling the gaps'? i.e. dt = 0.1, but effort set by year
...
Running \code{project()} returns an object of type \class{MizerSim}. Like the \class{MizerParams} class this is also made up of various ``slots'', which can be accessed using the \code{@} operator.

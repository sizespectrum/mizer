...
The result is an `ArraySpeciesByTime` (time × species) or `ArraySpeciesBySize`
...
`ArraySpeciesByTime` and have `plot()` and `ggplotly()` methods. This makes it
...
`plot(<ArraySpeciesByTime>)` accepts: `species`, `start_time`, `end_time`,

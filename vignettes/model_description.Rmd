...
individuals of species $i$ in the size interval $[w,w+dw]$. In other word: the
...
McKendrik-von Foerster equation, which is a transport equation (as one would
...
Besides the fish spectrum there is also a plankton spectrum $NPlankton(w)$,
...
You can retrieve these with `getPlanktonRate()` and `getPlanktonCapacity()`
respectively. It is also possible to implement other plankton dynamics, as
described in the help page for `setPlankton()`. The mortality $\mu_p(w)$ is
due to predation by consumers and isdescribed in the subsection 
[Plankton mortality].
...
The plankton spectrum is then represented by a vector `NPlankton` such that
`NPlankton[c]` =$N_p($`w_full[c]`$)$.
...
and the catabilities of different species.
...
The total rate of investment into reproduction (gramms/year) is
...
Two important density-dependent effects that in usual fisheries models are 
summarised in an imposed stock-recruitment relationship are already included
automatically in the mizer model:

1. High density of spawners leads to a reduced
food income of the spawners and consequently reduced per-capity reproduction.
...
modelled in mizer. An example would be a limited carrying capacity of suitable
spawning grounds and other spatial effects.
In mizer, any such density dependence is modelled as a compensation on the egg 
...
that the reproduction rate $R_i$ (numbers per time) approaches a maximum as the 

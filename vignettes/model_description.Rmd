...
Besides the size spectra of fish and plankton, mizer can also model 
unstructured components of the ecosystem.
Such unstructured components are appropriate whenever the
predation on this component is not size based. Examples include a detritus 
pool, biomass of dead animals, or biomass of plants on which fish can graze.
Mizer allows an arbitrary number of unstructured components. We will use
an index $d$ for these components. The biomass of the $d$th component is
denoted by $B_d$. 
The rate of change in biomasses is described by ODEs
\[\frac{d}{dt}B_d = \]
...
&+\sum_d\rho_{id}(w)\,B_d,
\end{split}\]
where we also included a term describing the feeding on the unstructured
resource components depending linearly on their biomasses $B_d$ with
rate coefficients $\rho_{id}(w)$.

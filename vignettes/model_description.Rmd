...
use for traffic density) but with an additional loss terms due to fish 
...
section. The emigration rate $e_i(w)$ is a rate at which fish leave the
size spectrum. It is set up with the `setEmigration()`
function and is by default zero.

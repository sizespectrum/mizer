...
Besides the fish spectrum there is also a plankton spectrum $N_R(w)$,
...
changed with `setPredationKernel()`.
...
\end{equation}
where $R_{\max.i}$ is the maximum reproduction rate of each trait class (Figure~\ref{fig:recruitment}). 
...
changed with `setExtMortality()`. By default mizer assumes an allometric form

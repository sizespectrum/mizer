...
capacity in the absence of predation. These parameters are set with
...
various prey species and plankton. It is set with `setInteraction()`.

The size selectivity is encoded in the predation kernel $\phi_i(w,w_p)$. This is
set with `setPredKernel()`.
...
where $h_i(w)$ is the maximum consumption rate. This is set with
...
activity and movement, at a rate $\text{metab}_i(w)$. By default 
...
\begin{equation}
  \label{eq:Er}
  E_{r.i}(w) = \max(0, \alpha_i f_i(w)\, h_i(w) - \text{metab}_i(w))
\end{equation}
...
all available energy is used for reproduction. This function is set with
...
set with `setBMort()`. By default mizer assumes an allometric form
...
\begin{equation}
  \label{eq:mupp}
  \mu_{p}(w_p) = \sum_j \text{pred_rate}_j(w_p)\, \theta_{jp}.
\end{equation}

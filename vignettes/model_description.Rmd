...
Sometimes one does not know all the parameters, and some parameters may be estimated 
by assuming they follow theoretical relationships. This leads to various additional equations, 
which are not part of essential core of mizer, but may be used to fill out missing parameter values. 
We shall discuss such equations here

The default parameter values for the model have been derived on the basis of meta-analyses of data on marine fish. Two parameters deserving special mention are the initial and critical feeding levels. These are used as physiological measures characterizing the productivity of the system and the metabolic requirements of the individuals.
...
## Volumetric search rate equation {#eq:gamma}
...
<!-- Similar to the way the initial feeding level $f_0$ is used to describe the relation between the production and the maximum consumption, the critical feeding level $f_c$ (Equation~\ref{eq:fc}) is used to describe the relation between the factors for standard metabolism $k_s$ and maximum consumption $h$. A critical feeding level of $f_c \approx 0.2$ seems to be reasonable.

...
integrating over all prey sizes $w_p$, weighted by the size selection function
$\phi(w,w_p)$,
\begin{equation}
  \label{eq:1}
...
could be interpreted as a search volume and is stored in an array
`search_vol[i,a]` where the first index specifies the species and the second the
weight of the predator.

This encouter rate is calculated by the `getEncounter()` function.
...
consumed is then $(1-f_i(w))E_{i}(w)=f_i(w)\, h_i(w)$.

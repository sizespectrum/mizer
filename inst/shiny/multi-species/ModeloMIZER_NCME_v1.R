...
# ---------------------------
# Setting the Fishing effort (E)
# ---------------------------
# Catchability is a scalar parameter used to modify the Fishing mortality at size given the selectivity at size and effort
# of the fishing gear. (Mllanote: " a bit tricky how the effort is implemented here", it is more easy to understand if I just set q=1")

# The catchability can be set by including a catchability column in the species parameters data.frame.
# (Mlla_notes: this is in the case that you want to set q differente than 1, that is defaults values)
# Doing this overwrites the default values when the MizerParams constructor is called.
# params_data$catchability <- as.numeric(f_history["1990",])

# Fishing gears, selectivities and catchabilities
# There are 3 options to set E.

# 1. A single value of E (cte time and gears)ls

sim1<- project(params, effort = 1, t_max = 150, dt = 0.1, t_save = 0.1)
plot(sim1)
head(sim1@effort)
summary(sim1)
...
effort <- c(NF = 0, P = 0.5)
sim2<- project(params_gears, effort = effort, t_max = 600, dt = 1, t_save = 1)
plot(sim2)
head(sim2@effort)
summary(sim2)
plotFMort(sim2)

# 3. Changing effort throught time and gears (matrix t,g)

gear_names <- c("NF","P")
times <- seq(from = 100, to = 150, by = 1)
effort_array <- array(NA, dim = c(length(times), length(gear_names)), dimnames = list(time = times, gear = gear_names))
effort_array[,"NF"] <- 0
effort_array[,"P"] <- seq(from = 1, to = 2, length = length(times))
head(effort_array)
sim3<- project(params_gears, effort = effort_array, dt = 1, t_save = 1)
plot(sim3)
head(sim3@effort)
summary(sim3)
plotFMort(sim3)

# 4. Setting effort in a multispecies context (Selectivity function sigmoid shape)
params_data$sel_func <-"sigmoid"


# ----------------------------------------------
# 4) Exploring the Results   
# ----------------------------------------------
# MizerSim stores the results of the projections, in four slots?: i) params, ii) effort, iii) n, and iv) n_pp
ssb<-getSSB(sim3)  # biomass by sps and at each t_save
dim(ssb)
head(ssb)
N<-getN(sim3)      # abundance by sps and at each t_save

biomass <- getBiomass(sim3, min_w = 10, max_w = 1000) # We obtain the biomass of the indivual of body min_w and max_w (interval of body size)

# Some community slopes at each time
slope <-getCommunitySlope(sim3)    # whole community
dem_species <- c("Mesopelagic","Anchovy","Csardine","Mackerel","JMackerel")
slope <- getCommunitySlope(sim3, min_w = 10, max_w = 5000, species = dem_species)

# ----------------------------------------------
# 5) Plotting the Results   
# ----------------------------------------------
plotBiomass(sim3)
plotSpectra(sim3,time_range=5:10,biomass=TRUE)
plot(sim3)
plotM2(sim3)

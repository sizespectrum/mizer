...
#for (i in (1:3)){
  ...
  #  p <- addSpecies(p, species_params, SSB = 2800, 
   #                 effort = 0, rfac = 1.01)
    ...
    sim <- project(p, t_max = 100, t_save = 5, effort = effort)
    #plot(sim)
    #########
    p <- sim@params
...
p@species_params$r_max <- Inf
  ...
  for (i in 11:18){
  plotGrowthCurves(p, species=as.character(p@species_params$species[i]))}
...
# Just pushed testingForAppSourcingRetry2fish.R This unfished system seems unstable, but is improved when fishing
  ...
  #changed l50 of mesopelagic fish, for better fishing gear
  
  # I wanted to sumarize what we did today. I just pushed that code (testingForAppSourcingRetry2fishC.R ), 
  # in which we do a manual run after adding each species. Older code was   testingForAppSourcingRetry2fish.R (that worked, and did a (long way) manual run at the end, to get to a 
  # steady state). 
  # One problem with the new code was that the runs were not doing long enough. Before we also had the problem of confusing 
  # the Rmaxs. Also, we are now letting mizer set the SSB of the species. I think the code basically 
  # runs now I sorted out that problem with setting the Rmaxs. So I think a good thing to do soon will be 
  # for me to try and understand what is going wrong as the species are being added in. I can start by trying to understand 
  # why the first manual tunning sim for the mesopelagics did not converge.  
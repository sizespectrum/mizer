...
Your params objects carry a distinctive class label, an **S3 marker class** that
prepends your class name to `"MizerParams"` (e.g. `c("mizerShelf", "MizerParams")`)
but adds no new data. All extension-specific data lives in `other_params(params)` or
in component parameters; the class is only a label for S3 dispatch.

**Do not call `setClass()`.** Mizer manages the S3 class hierarchy for you.
`registerExtension()` recognises your package as a dispatching extension from the S3
methods it registers for its marker class — `getBiomass.mizerShelf()` and the like.
`coerceToExtensionClass()` then arranges the S3 class vector in dispatch order
relative to whatever other extensions are loaded.
...
#'
#' S3 marker classes for MizerParams and MizerSim that enable S3 dispatch for
#' the methods defined in this package. They add no fields and are managed by
#' mizer when the package is loaded.
#'

...
| `getSteadyResidual()` returns `NA` where it used to return minus a mortality rate | classes below `biomass_share_cutoff` are reported as holding nothing | A size class holding no fish no longer blocks convergence |

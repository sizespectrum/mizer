...
| `isSteady()` is now `TRUE`, or `residual` much smaller, on a model that registers a component with `setComponent()` (mizerMR, mizerReef, mizerShelf) | components are no longer folded into the biomass drift | Other components are no longer part of the steady-state criterion |
| Message `"Components are not included in the biomass drift"` | a component with dynamics of its own is drifting, and the drift reported beside it does not cover it | Other components are no longer part of the steady-state criterion |
| Message `"consumers and the resource in this model are at their steady state"` | the consumers have settled but a component has not | Other components are no longer part of the steady-state criterion |
| Message `"could not be determined, because mizer cannot difference"` | a component's state is not numeric, so it has no measurable rate of change | Other components are no longer part of the steady-state criterion |
| Message `"is changing at"` names a species or the resource, where the report used to say only that a biomass was changing | the drift report now identifies the state variable responsible | Other components are no longer part of the steady-state criterion |
...
| After `rm(list = ls())`, or in the second and later examples of an extension package's `R CMD check`, mizer reports that the extension's class is not a defined class | the dynamic marker classes lived in `.GlobalEnv` and were wiped along with it | Extension marker classes survive a cleared workspace |
...
| Error ``"Could not calculate a default `gamma` for the following species:"`` from `species_params<-()` or `upgradeParams()` on an extension model | the extension zeroed the encounter rate for a species while the `gamma` default was being measured through it | Defaults for `gamma` and `f0` ignore the extension chain |
...
| A recalculated `f0` is lower on a model with external encounter, `other_encounter()` or a component `encounter_fun`, or `f0` → `gamma` → `f0` now round-trips | additive encounter contributions are no longer part of the power-law reference state | `gamma` and `f0` defaults ignore additive encounter contributions |
| Error ``"Could not calculate a default `gamma` for the following species:"`` on a model that builds a species with `interaction_resource = 0` and an external, free-standing or component encounter | that additive contribution used to stand in for the missing predation encounter and yielded a `gamma` many orders of magnitude too large | `gamma` and `f0` defaults ignore additive encounter contributions |

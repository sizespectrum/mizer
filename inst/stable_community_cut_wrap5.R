...
#19 w-grid is automatically setup to include points 
# min_egg=min_w and max_w_inf=max_w, and now we 
# have added code to coerce the other characteristic 
# weight inputs (min_w_inf and min_w_mat) so they also lie 
# on grid points. Now just have to tidy up the code.
...
#19 got code which sets up sizes better, but it runs slights 
# differently, probably due to rounding errors. 
# one task is to decide if this or the wrap2 code is better. 
# Another task is to sort out the way w_pp is handled here.
...
set_scaling_model <- function(f0 = 0.6,
  ...
  #min_egg and #max_w already lie on grid points, this ensures 
  # that min_w_mat lies on a grid point.
  ...
  #also ensure that min_w_inf lies on a grid point

...
#'
#' Note that the result has a single time step, so a function that needs more
#' than one — anything taking a difference through time — cannot be applied to
#' it.
#'
#' @param params A MizerParams object.
#' @param t The time to label the single time step with.
#' @return A MizerSim object with one time step holding the initial state of
#'   `params`.
#' @keywords internal
params_as_sim <- function(params, t = 0) {
    sim <- MizerSim(params, t_dimnames = t)
    sim@n[1, , ] <- params@initial_n
    sim@n_pp[1, ] <- params@initial_n_pp
    if (length(params@initial_n_other) > 0) {

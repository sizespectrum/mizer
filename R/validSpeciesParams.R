...
#' * the asymptotic size is not specified for all species
#'
...
#' inconsistent.
#'
#' If a `w_inf` column is given but no `w_repro_max` then the value from `w_inf`
#' is used. Note that the von Bertalanffy parameter `w_inf` is not the maximum
#' size of the largest individual, but the asymptotic size of an average
#' individual.
#'

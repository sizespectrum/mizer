# MizerRate S3 class for species x size arrays
#
...
#'
#' Many rate functions in mizer return two-dimensional arrays (species x size)
#' holding rates like encounter rate, feeding level, growth rate, mortality etc.
#' The `MizerRate` class wraps these arrays to provide convenient `print()`,
#' `summary()`, `plot()`, and `as.data.frame()` methods.
#'
#' A `MizerRate` object behaves just like a regular matrix for arithmetic
#' operations and subsetting. It carries two lightweight attributes:
#' \itemize{
#'   \item `rate_name` – a human-readable name for the rate (e.g. "Encounter rate").
#'   \item `units` – the units of the rate (e.g. "g/year").
#' }
#'
#' @param x A matrix (species x size).
#' @param rate_name A string giving the human-readable name for the rate.
...
#'   future extensions.
#'
#' @return A `MizerRate` object (inherits from `matrix` and `array`).
#' @export
#' @examples
#' \donttest{
...
#' summary(enc)
#' }
MizerRate <- function(x, rate_name = NULL, units = NULL, params = NULL) {
    if (!is.matrix(x)) {
...
#' Test if an object is a MizerRate
#'
#' @param x An object to test.
#' @return `TRUE` if `x` is a `MizerRate` object, `FALSE` otherwise.
#' @export
#' @examples
#' is.MizerRate(getEncounter(NS_params))
#' is.MizerRate(matrix(1:4, nrow = 2))
is.MizerRate <- function(x) {
    inherits(x, "MizerRate")
}

#' @export
print.MizerRate <- function(x, ...) {
    rate_name <- attr(x, "rate_name") %||% "MizerRate"
    units_str <- attr(x, "units")
    dims <- dim(x)
    header <- paste0(rate_name, " (", dims[1], " species x ", dims[2], " sizes)")
...
#' @export
summary.MizerRate <- function(object, ...) {
    rate_name <- attr(object, "rate_name") %||% "MizerRate"
    ...
    class(result) <- "summary.MizerRate"
    result
}

#' @export
print.summary.MizerRate <- function(x, ...) {
    header <- x$rate_name
    if (!is.null(x$units)) {
...
#' Plot a MizerRate object
#'
#' Plots the rate against size for each species, using species colours and
#' linetypes from the MizerParams object.
#'
#' @param x A `MizerRate` object.
...
#' }
plot.MizerRate <- function(x, params = NULL, species = NULL, 
                            all.sizes = FALSE, highlight = NULL, 
                            return_data = FALSE, log_x = TRUE, ...) {
    rate_name <- attr(x, "rate_name") %||% "Rate"
    units_str <- attr(x, "units")
    ...
    y_label <- rate_name
    if (!is.null(units_str)) {
        y_label <- paste0(rate_name, " [", units_str, "]")
    }
    ...
    if (!is.null(linecolour)) {
        p <- p + ggplot2::scale_colour_manual(values = linecolour[legend_levels])
    }
    if (!is.null(linetype)) {
        p <- p + ggplot2::scale_linetype_manual(values = linetype[legend_levels])
    }
...
#' @export
as.data.frame.MizerRate <- function(x, row.names = NULL,
                                     optional = FALSE, ...) {
    w <- as.numeric(colnames(x))
    if (any(is.na(w))) {
        w <- seq_len(ncol(x))
...
#' @export
`[.MizerRate` <- function(x, i, j, ..., drop = TRUE) {
    result <- NextMethod()
    # Preserve class only if result is still a 2D matrix
    if (is.matrix(result) && length(dim(result)) == 2) {
        attr(result, "rate_name") <- attr(x, "rate_name")
        attr(result, "units") <- attr(x, "units")
        class(result) <- c("MizerRate", "matrix", "array")
    }
    result
}

#' @export
Ops.MizerRate <- function(e1, e2) {
    # Strip MizerRate class so that arithmetic returns a plain matrix.
    # We unclass both operands and call the generic directly.
    if (is.MizerRate(e1)) e1 <- unclass_rate(e1)
    if (!missing(e2) && is.MizerRate(e2)) e2 <- unclass_rate(e2)
    op <- match.fun(.Generic)
    if (missing(e2)) op(e1) else op(e1, e2)
}

# Helper to strip all MizerRate attributes
unclass_rate <- function(x) {
    x <- unclass(x)
    attr(x, "rate_name") <- NULL
    attr(x, "units") <- NULL
    x
}

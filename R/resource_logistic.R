...
balance_resource_logistic <- function(params,
    ...
    if (!is.null(resource_rate)) {
        ...
        death <- (mu * NR) != 0
        at_capacity <- death & resource_capacity == NR
        if (any(at_capacity)) {
            # Large enough to survive rounding while still being tiny.
            factor <- 1 + sqrt(.Machine$double.eps)
            warning(
                "The resource capacity has been increased slightly to lie ",
                "above the current resource abundance to avoid division by zero.",
                call. = FALSE
            )
